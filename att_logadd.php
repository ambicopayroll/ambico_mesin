...
$att_log_add = NULL; // Initialize page object first

class catt_log_add extends catt_log {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["scan_date"] != "") {
				...
				$this->setKey("scan_date", $this->scan_date->CurrentValue); // Set up key
			} else {
				$this->setKey("scan_date", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if (@$_GET["pin"] != "") {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "att_logview.php")
	...
	function LoadDefaultValues() {
		...
		$this->scan_date->CurrentValue = NULL;
		$this->scan_date->OldValue = $this->scan_date->CurrentValue;
		$this->pin->CurrentValue = NULL;
		$this->pin->OldValue = $this->pin->CurrentValue;
		$this->verifymode->CurrentValue = NULL;
		$this->verifymode->OldValue = $this->verifymode->CurrentValue;
		$this->inoutmode->CurrentValue = 0;
		$this->reserved->CurrentValue = 0;
		$this->work_code->CurrentValue = 0;
		$this->att_id->CurrentValue = "0";
	}
	...
	function RenderRow() {
		...
		$this->att_id->ViewCustomAttributes = "";
			...
			$this->scan_date->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->scan_date->CurrentValue, 8));
			$this->scan_date->PlaceHolder = ew_RemoveHtml($this->scan_date->FldCaption());
	...
	function AddRow($rsold = NULL) {
		...
		// scan_date
		$this->scan_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->scan_date->CurrentValue, 0), ew_CurrentDate(), FALSE);

		// pin
		$this->pin->SetDbValueDef($rsnew, $this->pin->CurrentValue, "", FALSE);

		// verifymode
		$this->verifymode->SetDbValueDef($rsnew, $this->verifymode->CurrentValue, 0, FALSE);

		// inoutmode
		$this->inoutmode->SetDbValueDef($rsnew, $this->inoutmode->CurrentValue, 0, strval($this->inoutmode->CurrentValue) == "");

		// reserved
		$this->reserved->SetDbValueDef($rsnew, $this->reserved->CurrentValue, 0, strval($this->reserved->CurrentValue) == "");

		// work_code
		$this->work_code->SetDbValueDef($rsnew, $this->work_code->CurrentValue, 0, strval($this->work_code->CurrentValue) == "");

		// att_id
		$this->att_id->SetDbValueDef($rsnew, $this->att_id->CurrentValue, "", strval($this->att_id->CurrentValue) == "");
		...
		// Check if key value entered
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['scan_date']) == "") {
			$this->setFailureMessage($Language->Phrase("InvalidKeyValue"));
			$bInsertRow = FALSE;
		}
...
// Create page object
if (!isset($att_log_add)) $att_log_add = new catt_log_add();

// Page init
$att_log_add->Page_Init();

// Page main
$att_log_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$att_log_add->Page_Render();
?>
...
var CurrentForm = fatt_logadd = new ew_Form("fatt_logadd", "add");

// Validate form
fatt_logadd.Validate = function() {
...
fatt_logadd.Form_CustomValidate = 
...
fatt_logadd.ValidateRequired = true;
<?php } else { ?>
fatt_logadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $att_log_add->ShowPageHeader(); ?>
<?php
$att_log_add->ShowMessage();
?>
<form name="fatt_logadd" id="fatt_logadd" class="<?php echo $att_log_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($att_log_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $att_log_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="att_log" data-field="x_sn" name="x_sn" id="x_sn" size="30" maxlength="30" placeholder="<?php echo ew_HtmlEncode($att_log->sn->getPlaceHolder()) ?>" value="<?php echo $att_log->sn->EditValue ?>"<?php echo $att_log->sn->EditAttributes() ?>>
</span>
...
<input type="text" data-table="att_log" data-field="x_scan_date" name="x_scan_date" id="x_scan_date" placeholder="<?php echo ew_HtmlEncode($att_log->scan_date->getPlaceHolder()) ?>" value="<?php echo $att_log->scan_date->EditValue ?>"<?php echo $att_log->scan_date->EditAttributes() ?>>
</span>
...
<input type="text" data-table="att_log" data-field="x_pin" name="x_pin" id="x_pin" size="30" maxlength="32" placeholder="<?php echo ew_HtmlEncode($att_log->pin->getPlaceHolder()) ?>" value="<?php echo $att_log->pin->EditValue ?>"<?php echo $att_log->pin->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $att_log_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
