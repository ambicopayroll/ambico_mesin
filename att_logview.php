...
$att_log_view = NULL; // Initialize page object first

class catt_log_view extends catt_log {
	...
	function __construct() {
		...
		if (@$_GET["scan_date"] <> "") {
			$this->RecKey["scan_date"] = $_GET["scan_date"];
			$KeyUrl .= "&amp;scan_date=" . urlencode($this->RecKey["scan_date"]);
		}
		if (@$_GET["pin"] <> "") {
	...
	function Page_Init() {
		...
		if (@$_GET["scan_date"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
			$gsExportFile .= ew_StripSlashes($_GET["scan_date"]);
		}
		if (@$_GET["pin"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["sn"] <> "") {
				...
				$sReturnUrl = "att_loglist.php"; // Return to list
			}
			if (@$_GET["scan_date"] <> "") {
				$this->scan_date->setQueryStringValue($_GET["scan_date"]);
				$this->RecKey["scan_date"] = $this->scan_date->QueryStringValue;
			} elseif (@$_POST["scan_date"] <> "") {
				$this->scan_date->setFormValue($_POST["scan_date"]);
				$this->RecKey["scan_date"] = $this->scan_date->FormValue;
			} else {
				$sReturnUrl = "att_loglist.php"; // Return to list
			}
			if (@$_GET["pin"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "att_loglist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "att_loglist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($att_log_view)) $att_log_view = new catt_log_view();

// Page init
$att_log_view->Page_Init();

// Page main
$att_log_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$att_log_view->Page_Render();
?>
...
var CurrentForm = fatt_logview = new ew_Form("fatt_logview", "view");

// Form_CustomValidate event
fatt_logview.Form_CustomValidate = 
...
fatt_logview.ValidateRequired = true;
<?php } else { ?>
fatt_logview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($att_log->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$att_log_view->IsModal) { ?>
<?php if ($att_log->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $att_log_view->ExportOptions->Render("body") ?>
<?php
	foreach ($att_log_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$att_log_view->IsModal) { ?>
<?php if ($att_log->Export == "") { ?>
...
<?php } ?>
<?php $att_log_view->ShowPageHeader(); ?>
<?php
$att_log_view->ShowMessage();
?>
<form name="fatt_logview" id="fatt_logview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($att_log_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $att_log_view->Token ?>">
<?php } ?>
...
<?php if ($att_log->scan_date->Visible) { // scan_date ?>
	<tr id="r_scan_date">
		<td><span id="elh_att_log_scan_date"><?php echo $att_log->scan_date->FldCaption() ?></span></td>
...
<?php echo $att_log->scan_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($att_log->verifymode->Visible) { // verifymode ?>
	<tr id="r_verifymode">
		<td><span id="elh_att_log_verifymode"><?php echo $att_log->verifymode->FldCaption() ?></span></td>
...
<?php echo $att_log->verifymode->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($att_log->inoutmode->Visible) { // inoutmode ?>
	<tr id="r_inoutmode">
		<td><span id="elh_att_log_inoutmode"><?php echo $att_log->inoutmode->FldCaption() ?></span></td>
...
<?php echo $att_log->inoutmode->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($att_log->reserved->Visible) { // reserved ?>
	<tr id="r_reserved">
		<td><span id="elh_att_log_reserved"><?php echo $att_log->reserved->FldCaption() ?></span></td>
...
<?php echo $att_log->reserved->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($att_log->work_code->Visible) { // work_code ?>
	<tr id="r_work_code">
		<td><span id="elh_att_log_work_code"><?php echo $att_log->work_code->FldCaption() ?></span></td>
...
<?php echo $att_log->work_code->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($att_log->att_id->Visible) { // att_id ?>
	<tr id="r_att_id">
		<td><span id="elh_att_log_att_id"><?php echo $att_log->att_id->FldCaption() ?></span></td>
...
<?php echo $att_log->att_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
