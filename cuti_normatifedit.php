...
$cuti_normatif_edit = NULL; // Initialize page object first

class ccuti_normatif_edit extends ccuti_normatif {
	...
	// Page object name
	var $PageObjName = 'cuti_normatif_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->cuti_n_id->CurrentValue == "") {
			$this->Page_Terminate("cuti_normatiflist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->cuti_n_nama->SetDbValueDef($rsnew, $this->cuti_n_nama->CurrentValue, "", $this->cuti_n_nama->ReadOnly);

			// cuti_n_lama
			$this->cuti_n_lama->SetDbValueDef($rsnew, $this->cuti_n_lama->CurrentValue, 0, $this->cuti_n_lama->ReadOnly);

			// nominal
			$this->nominal->SetDbValueDef($rsnew, $this->nominal->CurrentValue, 0, $this->nominal->ReadOnly);

			// jns_bayar
			$this->jns_bayar->SetDbValueDef($rsnew, $this->jns_bayar->CurrentValue, 0, $this->jns_bayar->ReadOnly);
...
// Create page object
if (!isset($cuti_normatif_edit)) $cuti_normatif_edit = new ccuti_normatif_edit();

// Page init
$cuti_normatif_edit->Page_Init();

// Page main
$cuti_normatif_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cuti_normatif_edit->Page_Render();
?>
...
var CurrentForm = fcuti_normatifedit = new ew_Form("fcuti_normatifedit", "edit");

// Validate form
fcuti_normatifedit.Validate = function() {
...
fcuti_normatifedit.Form_CustomValidate = 
...
fcuti_normatifedit.ValidateRequired = true;
<?php } else { ?>
fcuti_normatifedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$cuti_normatif_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $cuti_normatif_edit->ShowPageHeader(); ?>
<?php
$cuti_normatif_edit->ShowMessage();
?>
<form name="fcuti_normatifedit" id="fcuti_normatifedit" class="<?php echo $cuti_normatif_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cuti_normatif_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cuti_normatif_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $cuti_normatif->cuti_n_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="cuti_normatif" data-field="x_cuti_n_id" name="x_cuti_n_id" id="x_cuti_n_id" value="<?php echo ew_HtmlEncode($cuti_normatif->cuti_n_id->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $cuti_normatif_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fcuti_normatifedit.Init();
</script>
<?php
$cuti_normatif_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$cuti_normatif_edit->Page_Terminate();
?>
