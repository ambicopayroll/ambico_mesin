...
$cuti_normatif_view = NULL; // Initialize page object first

class ccuti_normatif_view extends ccuti_normatif {
	...
	// Page object name
	var $PageObjName = 'cuti_normatif_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["cuti_n_id"] <> "") {
			$this->RecKey["cuti_n_id"] = $_GET["cuti_n_id"];
			$KeyUrl .= "&amp;cuti_n_id=" . urlencode($this->RecKey["cuti_n_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["cuti_n_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["cuti_n_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["cuti_n_id"] <> "") {
				...
				$this->RecKey["cuti_n_id"] = $this->cuti_n_id->QueryStringValue;
			} elseif (@$_POST["cuti_n_id"] <> "") {
				$this->cuti_n_id->setFormValue($_POST["cuti_n_id"]);
				$this->RecKey["cuti_n_id"] = $this->cuti_n_id->FormValue;
			} else {
				$sReturnUrl = "cuti_normatiflist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "cuti_normatiflist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "cuti_normatiflist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($cuti_normatif_view)) $cuti_normatif_view = new ccuti_normatif_view();

// Page init
$cuti_normatif_view->Page_Init();

// Page main
$cuti_normatif_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$cuti_normatif_view->Page_Render();
?>
...
var CurrentForm = fcuti_normatifview = new ew_Form("fcuti_normatifview", "view");

// Form_CustomValidate event
fcuti_normatifview.Form_CustomValidate = 
...
fcuti_normatifview.ValidateRequired = true;
<?php } else { ?>
fcuti_normatifview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($cuti_normatif->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$cuti_normatif_view->IsModal) { ?>
<?php if ($cuti_normatif->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $cuti_normatif_view->ExportOptions->Render("body") ?>
<?php
	foreach ($cuti_normatif_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$cuti_normatif_view->IsModal) { ?>
<?php if ($cuti_normatif->Export == "") { ?>
...
<?php } ?>
<?php $cuti_normatif_view->ShowPageHeader(); ?>
<?php
$cuti_normatif_view->ShowMessage();
?>
<form name="fcuti_normatifview" id="fcuti_normatifview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($cuti_normatif_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $cuti_normatif_view->Token ?>">
<?php } ?>
...
<?php if ($cuti_normatif->cuti_n_id->Visible) { // cuti_n_id ?>
	<tr id="r_cuti_n_id">
		<td><span id="elh_cuti_normatif_cuti_n_id"><?php echo $cuti_normatif->cuti_n_id->FldCaption() ?></span></td>
...
<?php echo $cuti_normatif->cuti_n_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cuti_normatif->cuti_n_nama->Visible) { // cuti_n_nama ?>
	<tr id="r_cuti_n_nama">
		<td><span id="elh_cuti_normatif_cuti_n_nama"><?php echo $cuti_normatif->cuti_n_nama->FldCaption() ?></span></td>
...
<?php echo $cuti_normatif->cuti_n_nama->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cuti_normatif->cuti_n_lama->Visible) { // cuti_n_lama ?>
	<tr id="r_cuti_n_lama">
		<td><span id="elh_cuti_normatif_cuti_n_lama"><?php echo $cuti_normatif->cuti_n_lama->FldCaption() ?></span></td>
...
<?php echo $cuti_normatif->cuti_n_lama->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cuti_normatif->nominal->Visible) { // nominal ?>
	<tr id="r_nominal">
		<td><span id="elh_cuti_normatif_nominal"><?php echo $cuti_normatif->nominal->FldCaption() ?></span></td>
...
<?php echo $cuti_normatif->nominal->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($cuti_normatif->jns_bayar->Visible) { // jns_bayar ?>
	<tr id="r_jns_bayar">
		<td><span id="elh_cuti_normatif_jns_bayar"><?php echo $cuti_normatif->jns_bayar->FldCaption() ?></span></td>
...
<?php echo $cuti_normatif->jns_bayar->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($cuti_normatif->Export == "") { ?>
<script type="text/javascript">
fcuti_normatifview.Init();
</script>
<?php } ?>
<?php
$cuti_normatif_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($cuti_normatif->Export == "") { ?>
...
<?php
$cuti_normatif_view->Page_Terminate();
?>
