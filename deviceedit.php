...
$device_edit = NULL; // Initialize page object first

class cdevice_edit extends cdevice {
	...
	function Page_Main() {
		...
		if ($this->sn->CurrentValue == "") {
			$this->Page_Terminate("devicelist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// act_code_realtime
			$this->act_code_realtime->SetDbValueDef($rsnew, $this->act_code_realtime->CurrentValue, NULL, $this->act_code_realtime->ReadOnly);

			// device_name
			$this->device_name->SetDbValueDef($rsnew, $this->device_name->CurrentValue, NULL, $this->device_name->ReadOnly);

			// comm_key
			$this->comm_key->SetDbValueDef($rsnew, $this->comm_key->CurrentValue, NULL, $this->comm_key->ReadOnly);

			// dev_id
			$this->dev_id->SetDbValueDef($rsnew, $this->dev_id->CurrentValue, 0, $this->dev_id->ReadOnly);

			// comm_type
			$this->comm_type->SetDbValueDef($rsnew, $this->comm_type->CurrentValue, 0, $this->comm_type->ReadOnly);

			// ip_address
			$this->ip_address->SetDbValueDef($rsnew, $this->ip_address->CurrentValue, NULL, $this->ip_address->ReadOnly);

			// id_type
			$this->id_type->SetDbValueDef($rsnew, $this->id_type->CurrentValue, 0, $this->id_type->ReadOnly);

			// dev_type
			$this->dev_type->SetDbValueDef($rsnew, $this->dev_type->CurrentValue, 0, $this->dev_type->ReadOnly);

			// serial_port
			$this->serial_port->SetDbValueDef($rsnew, $this->serial_port->CurrentValue, NULL, $this->serial_port->ReadOnly);

			// baud_rate
			$this->baud_rate->SetDbValueDef($rsnew, $this->baud_rate->CurrentValue, NULL, $this->baud_rate->ReadOnly);

			// ethernet_port
			$this->ethernet_port->SetDbValueDef($rsnew, $this->ethernet_port->CurrentValue, "", $this->ethernet_port->ReadOnly);

			// layar
			$this->layar->SetDbValueDef($rsnew, $this->layar->CurrentValue, 0, $this->layar->ReadOnly);

			// alg_ver
			$this->alg_ver->SetDbValueDef($rsnew, $this->alg_ver->CurrentValue, 0, $this->alg_ver->ReadOnly);

			// use_realtime
			$this->use_realtime->SetDbValueDef($rsnew, $this->use_realtime->CurrentValue, 0, $this->use_realtime->ReadOnly);

			// group_realtime
			$this->group_realtime->SetDbValueDef($rsnew, $this->group_realtime->CurrentValue, 0, $this->group_realtime->ReadOnly);

			// last_download
			$this->last_download->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->last_download->CurrentValue, 0), NULL, $this->last_download->ReadOnly);

			// ATTLOGStamp
			$this->ATTLOGStamp->SetDbValueDef($rsnew, $this->ATTLOGStamp->CurrentValue, "", $this->ATTLOGStamp->ReadOnly);

			// OPERLOGStamp
			$this->OPERLOGStamp->SetDbValueDef($rsnew, $this->OPERLOGStamp->CurrentValue, "", $this->OPERLOGStamp->ReadOnly);

			// ATTPHOTOStamp
			$this->ATTPHOTOStamp->SetDbValueDef($rsnew, $this->ATTPHOTOStamp->CurrentValue, "", $this->ATTPHOTOStamp->ReadOnly);
...
// Create page object
if (!isset($device_edit)) $device_edit = new cdevice_edit();

// Page init
$device_edit->Page_Init();

// Page main
$device_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$device_edit->Page_Render();
?>
...
var CurrentForm = fdeviceedit = new ew_Form("fdeviceedit", "edit");

// Validate form
fdeviceedit.Validate = function() {
...
fdeviceedit.Form_CustomValidate = 
...
fdeviceedit.ValidateRequired = true;
<?php } else { ?>
fdeviceedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $device_edit->ShowPageHeader(); ?>
<?php
$device_edit->ShowMessage();
?>
<form name="fdeviceedit" id="fdeviceedit" class="<?php echo $device_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($device_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $device_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $device->sn->EditValue ?></p></span>
</span>
<input type="hidden" data-table="device" data-field="x_sn" name="x_sn" id="x_sn" value="<?php echo ew_HtmlEncode($device->sn->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $device_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
