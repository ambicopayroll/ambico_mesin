...
$grp_user_m_edit = NULL; // Initialize page object first

class cgrp_user_m_edit extends cgrp_user_m {
	...
	// Page object name
	var $PageObjName = 'grp_user_m_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->grp_user_id->CurrentValue == "") {
			$this->Page_Terminate("grp_user_mlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->grp_user_name->SetDbValueDef($rsnew, $this->grp_user_name->CurrentValue, "", $this->grp_user_name->ReadOnly);
			...
			// detector
			$this->detector->SetDbValueDef($rsnew, $this->detector->CurrentValue, "", $this->detector->ReadOnly);
...
// Create page object
if (!isset($grp_user_m_edit)) $grp_user_m_edit = new cgrp_user_m_edit();

// Page init
$grp_user_m_edit->Page_Init();

// Page main
$grp_user_m_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$grp_user_m_edit->Page_Render();
?>
...
var CurrentForm = fgrp_user_medit = new ew_Form("fgrp_user_medit", "edit");

// Validate form
fgrp_user_medit.Validate = function() {
...
fgrp_user_medit.Form_CustomValidate = 
...
fgrp_user_medit.ValidateRequired = true;
<?php } else { ?>
fgrp_user_medit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$grp_user_m_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $grp_user_m_edit->ShowPageHeader(); ?>
<?php
$grp_user_m_edit->ShowMessage();
?>
<form name="fgrp_user_medit" id="fgrp_user_medit" class="<?php echo $grp_user_m_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($grp_user_m_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $grp_user_m_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $grp_user_m->grp_user_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="grp_user_m" data-field="x_grp_user_id" name="x_grp_user_id" id="x_grp_user_id" value="<?php echo ew_HtmlEncode($grp_user_m->grp_user_id->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $grp_user_m_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fgrp_user_medit.Init();
</script>
<?php
$grp_user_m_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$grp_user_m_edit->Page_Terminate();
?>
