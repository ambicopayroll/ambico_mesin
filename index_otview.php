...
$index_ot_view = NULL; // Initialize page object first

class cindex_ot_view extends cindex_ot {
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["index_id"] <> "") {
			$this->RecKey["index_id"] = $_GET["index_id"];
			$KeyUrl .= "&amp;index_id=" . urlencode($this->RecKey["index_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["index_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["index_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["index_id"] <> "") {
				...
				$this->RecKey["index_id"] = $this->index_id->QueryStringValue;
			} elseif (@$_POST["index_id"] <> "") {
				$this->index_id->setFormValue($_POST["index_id"]);
				$this->RecKey["index_id"] = $this->index_id->FormValue;
			} else {
				$sReturnUrl = "index_otlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "index_otlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "index_otlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($index_ot_view)) $index_ot_view = new cindex_ot_view();

// Page init
$index_ot_view->Page_Init();

// Page main
$index_ot_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$index_ot_view->Page_Render();
?>
...
var CurrentForm = findex_otview = new ew_Form("findex_otview", "view");

// Form_CustomValidate event
findex_otview.Form_CustomValidate = 
...
findex_otview.ValidateRequired = true;
<?php } else { ?>
findex_otview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($index_ot->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$index_ot_view->IsModal) { ?>
<?php if ($index_ot->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $index_ot_view->ExportOptions->Render("body") ?>
<?php
	foreach ($index_ot_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$index_ot_view->IsModal) { ?>
<?php if ($index_ot->Export == "") { ?>
...
<?php } ?>
<?php $index_ot_view->ShowPageHeader(); ?>
<?php
$index_ot_view->ShowMessage();
?>
<form name="findex_otview" id="findex_otview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($index_ot_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $index_ot_view->Token ?>">
<?php } ?>
...
<?php if ($index_ot->index_id->Visible) { // index_id ?>
	<tr id="r_index_id">
		<td><span id="elh_index_ot_index_id"><?php echo $index_ot->index_id->FldCaption() ?></span></td>
...
<?php echo $index_ot->index_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($index_ot->type_ot->Visible) { // type_ot ?>
	<tr id="r_type_ot">
		<td><span id="elh_index_ot_type_ot"><?php echo $index_ot->type_ot->FldCaption() ?></span></td>
...
<?php echo $index_ot->type_ot->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($index_ot->from_ot->Visible) { // from_ot ?>
	<tr id="r_from_ot">
		<td><span id="elh_index_ot_from_ot"><?php echo $index_ot->from_ot->FldCaption() ?></span></td>
...
<?php echo $index_ot->from_ot->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($index_ot->to_ot->Visible) { // to_ot ?>
	<tr id="r_to_ot">
		<td><span id="elh_index_ot_to_ot"><?php echo $index_ot->to_ot->FldCaption() ?></span></td>
...
<?php echo $index_ot->to_ot->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($index_ot->multiplier->Visible) { // multiplier ?>
	<tr id="r_multiplier">
		<td><span id="elh_index_ot_multiplier"><?php echo $index_ot->multiplier->FldCaption() ?></span></td>
...
<?php echo $index_ot->multiplier->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
