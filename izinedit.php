...
$izin_edit = NULL; // Initialize page object first

class cizin_edit extends cizin {
	...
	function Page_Main() {
		...
		if ($this->pegawai_id->CurrentValue == "") {
			$this->Page_Terminate("izinlist.php"); // Invalid key, return to list
		}
		if ($this->izin_urutan->CurrentValue == "") {
			$this->Page_Terminate("izinlist.php"); // Invalid key, return to list
		}
		if ($this->izin_tgl->CurrentValue == "") {
			$this->Page_Terminate("izinlist.php"); // Invalid key, return to list
		}
		if ($this->izin_jenis_id->CurrentValue == "") {
			$this->Page_Terminate("izinlist.php"); // Invalid key, return to list
		}
		if ($this->izin_status->CurrentValue == "") {
			$this->Page_Terminate("izinlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->izin_tgl_pengajuan->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->izin_tgl_pengajuan->CurrentValue, 0), ew_CurrentDate(), $this->izin_tgl_pengajuan->ReadOnly);

			// izin_tgl
			// izin_jenis_id
			// izin_catatan

			$this->izin_catatan->SetDbValueDef($rsnew, $this->izin_catatan->CurrentValue, NULL, $this->izin_catatan->ReadOnly);

			// izin_status
			// izin_tinggal_t1

			$this->izin_tinggal_t1->SetDbValueDef($rsnew, $this->izin_tinggal_t1->CurrentValue, NULL, $this->izin_tinggal_t1->ReadOnly);

			// izin_tinggal_t2
			$this->izin_tinggal_t2->SetDbValueDef($rsnew, $this->izin_tinggal_t2->CurrentValue, NULL, $this->izin_tinggal_t2->ReadOnly);

			// cuti_n_id
			$this->cuti_n_id->SetDbValueDef($rsnew, $this->cuti_n_id->CurrentValue, NULL, $this->cuti_n_id->ReadOnly);

			// izin_ket_lain
			$this->izin_ket_lain->SetDbValueDef($rsnew, $this->izin_ket_lain->CurrentValue, NULL, $this->izin_ket_lain->ReadOnly);

			// izin_noscan_time
			$this->izin_noscan_time->SetDbValueDef($rsnew, $this->izin_noscan_time->CurrentValue, NULL, $this->izin_noscan_time->ReadOnly);

			// kat_izin_id
			$this->kat_izin_id->SetDbValueDef($rsnew, $this->kat_izin_id->CurrentValue, NULL, $this->kat_izin_id->ReadOnly);

			// ket_status
			$this->ket_status->SetDbValueDef($rsnew, $this->ket_status->CurrentValue, NULL, $this->ket_status->ReadOnly);
...
// Create page object
if (!isset($izin_edit)) $izin_edit = new cizin_edit();
...
var CurrentForm = fizinedit = new ew_Form("fizinedit", "edit");

// Validate form
fizinedit.Validate = function() {
...
fizinedit.ValidateRequired = true;
<?php } else { ?>
fizinedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $izin_edit->ShowPageHeader(); ?>
<?php
$izin_edit->ShowMessage();
?>
<form name="fizinedit" id="fizinedit" class="<?php echo $izin_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($izin_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $izin_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $izin->pegawai_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="izin" data-field="x_pegawai_id" name="x_pegawai_id" id="x_pegawai_id" value="<?php echo ew_HtmlEncode($izin->pegawai_id->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $izin->izin_urutan->EditValue ?></p></span>
</span>
<input type="hidden" data-table="izin" data-field="x_izin_urutan" name="x_izin_urutan" id="x_izin_urutan" value="<?php echo ew_HtmlEncode($izin->izin_urutan->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $izin->izin_tgl->EditValue ?></p></span>
</span>
<input type="hidden" data-table="izin" data-field="x_izin_tgl" name="x_izin_tgl" id="x_izin_tgl" value="<?php echo ew_HtmlEncode($izin->izin_tgl->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $izin->izin_jenis_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="izin" data-field="x_izin_jenis_id" name="x_izin_jenis_id" id="x_izin_jenis_id" value="<?php echo ew_HtmlEncode($izin->izin_jenis_id->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $izin->izin_status->EditValue ?></p></span>
</span>
<input type="hidden" data-table="izin" data-field="x_izin_status" name="x_izin_status" id="x_izin_status" value="<?php echo ew_HtmlEncode($izin->izin_status->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $izin_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
