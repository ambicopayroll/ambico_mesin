...
$jam_kerja_add = NULL; // Initialize page object first

class cjam_kerja_add extends cjam_kerja {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "jam_kerjaview.php")
	...
	function LoadDefaultValues() {
		...
		$this->jk_kode->OldValue = $this->jk_kode->CurrentValue;
		$this->use_set->CurrentValue = 0;
		$this->jk_bcin->CurrentValue = "00:00:00";
		$this->jk_cin->CurrentValue = 0;
		$this->jk_ecin->CurrentValue = 0;
		$this->jk_tol_late->CurrentValue = 0;
		$this->jk_use_ist->CurrentValue = 0;
		$this->jk_ist1->CurrentValue = "00:00:00";
		$this->jk_ist2->CurrentValue = "00:00:00";
		$this->jk_tol_early->CurrentValue = 0;
		$this->jk_bcout->CurrentValue = 0;
		$this->jk_cout->CurrentValue = 0;
		$this->jk_ecout->CurrentValue = "00:00:00";
		$this->use_eot->CurrentValue = 0;
		$this->min_eot->CurrentValue = 0;
		$this->max_eot->CurrentValue = 0;
		$this->reduce_eot->CurrentValue = 0;
		$this->jk_durasi->CurrentValue = 0;
		$this->jk_countas->CurrentValue = 0;
		$this->jk_min_countas->CurrentValue = 0;
		$this->jk_ket->CurrentValue = NULL;
		$this->jk_ket->OldValue = $this->jk_ket->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// jk_kode
		$this->jk_kode->SetDbValueDef($rsnew, $this->jk_kode->CurrentValue, "", FALSE);

		// use_set
		$this->use_set->SetDbValueDef($rsnew, $this->use_set->CurrentValue, 0, strval($this->use_set->CurrentValue) == "");

		// jk_bcin
		$this->jk_bcin->SetDbValueDef($rsnew, $this->jk_bcin->CurrentValue, ew_CurrentTime(), strval($this->jk_bcin->CurrentValue) == "");

		// jk_cin
		$this->jk_cin->SetDbValueDef($rsnew, $this->jk_cin->CurrentValue, 0, strval($this->jk_cin->CurrentValue) == "");

		// jk_ecin
		$this->jk_ecin->SetDbValueDef($rsnew, $this->jk_ecin->CurrentValue, 0, strval($this->jk_ecin->CurrentValue) == "");

		// jk_tol_late
		$this->jk_tol_late->SetDbValueDef($rsnew, $this->jk_tol_late->CurrentValue, 0, strval($this->jk_tol_late->CurrentValue) == "");

		// jk_use_ist
		$this->jk_use_ist->SetDbValueDef($rsnew, $this->jk_use_ist->CurrentValue, 0, strval($this->jk_use_ist->CurrentValue) == "");

		// jk_ist1
		$this->jk_ist1->SetDbValueDef($rsnew, $this->jk_ist1->CurrentValue, ew_CurrentTime(), strval($this->jk_ist1->CurrentValue) == "");

		// jk_ist2
		$this->jk_ist2->SetDbValueDef($rsnew, $this->jk_ist2->CurrentValue, ew_CurrentTime(), strval($this->jk_ist2->CurrentValue) == "");

		// jk_tol_early
		$this->jk_tol_early->SetDbValueDef($rsnew, $this->jk_tol_early->CurrentValue, 0, strval($this->jk_tol_early->CurrentValue) == "");

		// jk_bcout
		$this->jk_bcout->SetDbValueDef($rsnew, $this->jk_bcout->CurrentValue, 0, strval($this->jk_bcout->CurrentValue) == "");

		// jk_cout
		$this->jk_cout->SetDbValueDef($rsnew, $this->jk_cout->CurrentValue, 0, strval($this->jk_cout->CurrentValue) == "");

		// jk_ecout
		$this->jk_ecout->SetDbValueDef($rsnew, $this->jk_ecout->CurrentValue, ew_CurrentTime(), strval($this->jk_ecout->CurrentValue) == "");

		// use_eot
		$this->use_eot->SetDbValueDef($rsnew, $this->use_eot->CurrentValue, 0, strval($this->use_eot->CurrentValue) == "");

		// min_eot
		$this->min_eot->SetDbValueDef($rsnew, $this->min_eot->CurrentValue, 0, strval($this->min_eot->CurrentValue) == "");

		// max_eot
		$this->max_eot->SetDbValueDef($rsnew, $this->max_eot->CurrentValue, 0, strval($this->max_eot->CurrentValue) == "");

		// reduce_eot
		$this->reduce_eot->SetDbValueDef($rsnew, $this->reduce_eot->CurrentValue, 0, strval($this->reduce_eot->CurrentValue) == "");

		// jk_durasi
		$this->jk_durasi->SetDbValueDef($rsnew, $this->jk_durasi->CurrentValue, 0, strval($this->jk_durasi->CurrentValue) == "");

		// jk_countas
		$this->jk_countas->SetDbValueDef($rsnew, $this->jk_countas->CurrentValue, 0, strval($this->jk_countas->CurrentValue) == "");

		// jk_min_countas
		$this->jk_min_countas->SetDbValueDef($rsnew, $this->jk_min_countas->CurrentValue, 0, strval($this->jk_min_countas->CurrentValue) == "");
...
// Create page object
if (!isset($jam_kerja_add)) $jam_kerja_add = new cjam_kerja_add();

// Page init
$jam_kerja_add->Page_Init();

// Page main
$jam_kerja_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$jam_kerja_add->Page_Render();
?>
...
var CurrentForm = fjam_kerjaadd = new ew_Form("fjam_kerjaadd", "add");

// Validate form
fjam_kerjaadd.Validate = function() {
...
fjam_kerjaadd.Form_CustomValidate = 
...
fjam_kerjaadd.ValidateRequired = true;
<?php } else { ?>
fjam_kerjaadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $jam_kerja_add->ShowPageHeader(); ?>
<?php
$jam_kerja_add->ShowMessage();
?>
<form name="fjam_kerjaadd" id="fjam_kerjaadd" class="<?php echo $jam_kerja_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($jam_kerja_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $jam_kerja_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $jam_kerja_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
