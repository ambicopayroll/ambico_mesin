...
$jatah_cuti_add = NULL; // Initialize page object first

class cjatah_cuti_add extends cjatah_cuti {
	...
	// Page object name
	var $PageObjName = 'jatah_cuti_add';
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["jatah_c_mulai"] != "") {
				...
				$this->setKey("jatah_c_mulai", $this->jatah_c_mulai->CurrentValue); // Set up key
			} else {
				$this->setKey("jatah_c_mulai", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if (@$_GET["jatah_c_akhir"] != "") {
				$this->jatah_c_akhir->setQueryStringValue($_GET["jatah_c_akhir"]);
				$this->setKey("jatah_c_akhir", $this->jatah_c_akhir->CurrentValue); // Set up key
			} else {
				$this->setKey("jatah_c_akhir", ""); // Clear key
				$this->CopyRecord = FALSE;
			}
			if ($this->CopyRecord) {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "jatah_cutiview.php")
	...
	function LoadDefaultValues() {
		...
		$this->jatah_c_mulai->CurrentValue = "0000-00-00";
		$this->jatah_c_akhir->CurrentValue = "0000-00-00";
		$this->jatah_c_jml->CurrentValue = 0;
		$this->jatah_c_hak_jml->CurrentValue = 0;
		$this->jatah_c_ambil_jml->CurrentValue = 0;
		$this->jatah_c_utang_jml->CurrentValue = 0;
	}
	...
	function RenderRow() {
		...
		$this->jatah_c_utang_jml->ViewCustomAttributes = "";
			...
			$this->jatah_c_mulai->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->jatah_c_mulai->CurrentValue, 8));
			$this->jatah_c_mulai->PlaceHolder = ew_RemoveHtml($this->jatah_c_mulai->FldCaption());

			// jatah_c_akhir
			$this->jatah_c_akhir->EditAttrs["class"] = "form-control";
			$this->jatah_c_akhir->EditCustomAttributes = "";
			$this->jatah_c_akhir->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->jatah_c_akhir->CurrentValue, 8));
			$this->jatah_c_akhir->PlaceHolder = ew_RemoveHtml($this->jatah_c_akhir->FldCaption());
	...
	function AddRow($rsold = NULL) {
		...
		// jatah_c_mulai
		$this->jatah_c_mulai->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->jatah_c_mulai->CurrentValue, 0), ew_CurrentDate(), strval($this->jatah_c_mulai->CurrentValue) == "");

		// jatah_c_akhir
		$this->jatah_c_akhir->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->jatah_c_akhir->CurrentValue, 0), ew_CurrentDate(), strval($this->jatah_c_akhir->CurrentValue) == "");

		// jatah_c_jml
		$this->jatah_c_jml->SetDbValueDef($rsnew, $this->jatah_c_jml->CurrentValue, NULL, strval($this->jatah_c_jml->CurrentValue) == "");

		// jatah_c_hak_jml
		$this->jatah_c_hak_jml->SetDbValueDef($rsnew, $this->jatah_c_hak_jml->CurrentValue, NULL, strval($this->jatah_c_hak_jml->CurrentValue) == "");

		// jatah_c_ambil_jml
		$this->jatah_c_ambil_jml->SetDbValueDef($rsnew, $this->jatah_c_ambil_jml->CurrentValue, NULL, strval($this->jatah_c_ambil_jml->CurrentValue) == "");

		// jatah_c_utang_jml
		$this->jatah_c_utang_jml->SetDbValueDef($rsnew, $this->jatah_c_utang_jml->CurrentValue, NULL, strval($this->jatah_c_utang_jml->CurrentValue) == "");
		...
		// Check if key value entered
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['jatah_c_mulai']) == "") {
			$this->setFailureMessage($Language->Phrase("InvalidKeyValue"));
			$bInsertRow = FALSE;
		}

		// Check if key value entered
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['jatah_c_akhir']) == "") {
			$this->setFailureMessage($Language->Phrase("InvalidKeyValue"));
			$bInsertRow = FALSE;
		}
...
// Create page object
if (!isset($jatah_cuti_add)) $jatah_cuti_add = new cjatah_cuti_add();

// Page init
$jatah_cuti_add->Page_Init();

// Page main
$jatah_cuti_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$jatah_cuti_add->Page_Render();
?>
...
var CurrentForm = fjatah_cutiadd = new ew_Form("fjatah_cutiadd", "add");

// Validate form
fjatah_cutiadd.Validate = function() {
...
fjatah_cutiadd.Form_CustomValidate = 
...
fjatah_cutiadd.ValidateRequired = true;
<?php } else { ?>
fjatah_cutiadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $jatah_cuti_add->ShowPageHeader(); ?>
<?php
$jatah_cuti_add->ShowMessage();
?>
<form name="fjatah_cutiadd" id="fjatah_cutiadd" class="<?php echo $jatah_cuti_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($jatah_cuti_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $jatah_cuti_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="jatah_cuti" data-field="x_pegawai_id" name="x_pegawai_id" id="x_pegawai_id" size="30" placeholder="<?php echo ew_HtmlEncode($jatah_cuti->pegawai_id->getPlaceHolder()) ?>" value="<?php echo $jatah_cuti->pegawai_id->EditValue ?>"<?php echo $jatah_cuti->pegawai_id->EditAttributes() ?>>
</span>
...
<input type="text" data-table="jatah_cuti" data-field="x_jatah_c_mulai" name="x_jatah_c_mulai" id="x_jatah_c_mulai" placeholder="<?php echo ew_HtmlEncode($jatah_cuti->jatah_c_mulai->getPlaceHolder()) ?>" value="<?php echo $jatah_cuti->jatah_c_mulai->EditValue ?>"<?php echo $jatah_cuti->jatah_c_mulai->EditAttributes() ?>>
</span>
...
<input type="text" data-table="jatah_cuti" data-field="x_jatah_c_akhir" name="x_jatah_c_akhir" id="x_jatah_c_akhir" placeholder="<?php echo ew_HtmlEncode($jatah_cuti->jatah_c_akhir->getPlaceHolder()) ?>" value="<?php echo $jatah_cuti->jatah_c_akhir->EditValue ?>"<?php echo $jatah_cuti->jatah_c_akhir->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $jatah_cuti_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
