...
$jdw_kerja_pegawai = NULL;

//
// Table class for jdw_kerja_pegawai
//
class cjdw_kerja_pegawai extends cTable {
	var $pegawai_id;
	var $jdw_kerja_m_id;
	...
	function __construct() {
		...
		$this->TableVar = 'jdw_kerja_pegawai';
		$this->TableName = 'jdw_kerja_pegawai';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`jdw_kerja_pegawai`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// pegawai_id
		$this->pegawai_id = new cField('jdw_kerja_pegawai', 'jdw_kerja_pegawai', 'x_pegawai_id', 'pegawai_id', '`pegawai_id`', '`pegawai_id`', 3, -1, FALSE, '`pegawai_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// jdw_kerja_m_id
		$this->jdw_kerja_m_id = new cField('jdw_kerja_pegawai', 'jdw_kerja_pegawai', 'x_jdw_kerja_m_id', 'jdw_kerja_m_id', '`jdw_kerja_m_id`', '`jdw_kerja_m_id`', 3, -1, FALSE, '`jdw_kerja_m_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// jdw_kerja_m_mulai
		$this->jdw_kerja_m_mulai = new cField('jdw_kerja_pegawai', 'jdw_kerja_pegawai', 'x_jdw_kerja_m_mulai', 'jdw_kerja_m_mulai', '`jdw_kerja_m_mulai`', ew_CastDateFieldForLike('`jdw_kerja_m_mulai`', 0, "DB"), 133, 0, FALSE, '`jdw_kerja_m_mulai`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`jdw_kerja_pegawai`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			...
			if (array_key_exists('jdw_kerja_m_mulai', $rs))
				ew_AddFilter($where, ew_QuotedName('jdw_kerja_m_mulai', $this->DBID) . '=' . ew_QuotedValue($rs['jdw_kerja_m_mulai'], $this->jdw_kerja_m_mulai->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`pegawai_id` = @pegawai_id@ AND `jdw_kerja_m_id` = @jdw_kerja_m_id@ AND `jdw_kerja_m_mulai` = '@jdw_kerja_m_mulai@'";
	}
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@jdw_kerja_m_mulai@", ew_AdjustSql(ew_UnFormatDateTime($this->jdw_kerja_m_mulai->CurrentValue,0), $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "jdw_kerja_pegawailist.php";
		}
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("jdw_kerja_pegawaiview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("jdw_kerja_pegawaiview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "jdw_kerja_pegawaiadd.php?" . $this->UrlParm($parm);
		else
			$url = "jdw_kerja_pegawaiadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("jdw_kerja_pegawaiedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("jdw_kerja_pegawaiadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("jdw_kerja_pegawaidelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		...
		$json .= ",jdw_kerja_m_mulai:" . ew_VarToJson($this->jdw_kerja_m_mulai->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		...
		if (!is_null($this->jdw_kerja_m_mulai->CurrentValue)) {
			$sUrl .= "&jdw_kerja_m_mulai=" . urlencode($this->jdw_kerja_m_mulai->CurrentValue);
		} else {
			return "javascript:ew_Alert(ewLanguage.Phrase('InvalidRecord'));";
		}
		return $sUrl;
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			elseif (isset($_GET["jdw_kerja_m_id"]))
				...
				$arKeys = NULL; // Do not setup
			if ($isPost && isset($_POST["jdw_kerja_m_mulai"]))
				$arKey[] = ew_StripSlashes($_POST["jdw_kerja_m_mulai"]);
			elseif (isset($_GET["jdw_kerja_m_mulai"]))
				$arKey[] = ew_StripSlashes($_GET["jdw_kerja_m_mulai"]);
			else
				$arKeys = NULL; // Do not setup
		...
		if (is_array($arKeys)) {
			foreach ($arKeys as $key) {
				...
				if (!is_numeric($key[0])) // pegawai_id
					continue;
				if (!is_numeric($key[1])) // jdw_kerja_m_id
					continue;
				$ar[] = $key;
			}
		}
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->jdw_kerja_m_id->CurrentValue = $key[1];
			$this->jdw_kerja_m_mulai->CurrentValue = $key[2];
