...
$kategori_izin_edit = NULL; // Initialize page object first

class ckategori_izin_edit extends ckategori_izin {
	...
	// Page object name
	var $PageObjName = 'kategori_izin_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->kat_izin_id->CurrentValue == "") {
			$this->Page_Terminate("kategori_izinlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->kat_izin_nama->SetDbValueDef($rsnew, $this->kat_izin_nama->CurrentValue, NULL, $this->kat_izin_nama->ReadOnly);

			// izin_jenis_id
			$this->izin_jenis_id->SetDbValueDef($rsnew, $this->izin_jenis_id->CurrentValue, NULL, $this->izin_jenis_id->ReadOnly);
...
// Create page object
if (!isset($kategori_izin_edit)) $kategori_izin_edit = new ckategori_izin_edit();

// Page init
$kategori_izin_edit->Page_Init();

// Page main
$kategori_izin_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kategori_izin_edit->Page_Render();
?>
...
var CurrentForm = fkategori_izinedit = new ew_Form("fkategori_izinedit", "edit");

// Validate form
fkategori_izinedit.Validate = function() {
...
fkategori_izinedit.Form_CustomValidate = 
...
fkategori_izinedit.ValidateRequired = true;
<?php } else { ?>
fkategori_izinedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$kategori_izin_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $kategori_izin_edit->ShowPageHeader(); ?>
<?php
$kategori_izin_edit->ShowMessage();
?>
<form name="fkategori_izinedit" id="fkategori_izinedit" class="<?php echo $kategori_izin_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kategori_izin_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kategori_izin_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $kategori_izin->kat_izin_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="kategori_izin" data-field="x_kat_izin_id" name="x_kat_izin_id" id="x_kat_izin_id" value="<?php echo ew_HtmlEncode($kategori_izin->kat_izin_id->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kategori_izin_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fkategori_izinedit.Init();
</script>
<?php
$kategori_izin_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kategori_izin_edit->Page_Terminate();
?>
