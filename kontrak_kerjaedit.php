...
$kontrak_kerja_edit = NULL; // Initialize page object first

class ckontrak_kerja_edit extends ckontrak_kerja {
	...
	// Page object name
	var $PageObjName = 'kontrak_kerja_edit';
	...
	function Page_Main() {
		...
		if ($this->pegawai_id->CurrentValue == "") {
			$this->Page_Terminate("kontrak_kerjalist.php"); // Invalid key, return to list
		}
		if ($this->kontrak_start->CurrentValue == "") {
			$this->Page_Terminate("kontrak_kerjalist.php"); // Invalid key, return to list
		}
		if ($this->kontrak_end->CurrentValue == "") {
			$this->Page_Terminate("kontrak_kerjalist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->kontrak_status->SetDbValueDef($rsnew, $this->kontrak_status->CurrentValue, NULL, $this->kontrak_status->ReadOnly);

			// kontrak_aktif
			$this->kontrak_aktif->SetDbValueDef($rsnew, $this->kontrak_aktif->CurrentValue, NULL, $this->kontrak_aktif->ReadOnly);
...
// Create page object
if (!isset($kontrak_kerja_edit)) $kontrak_kerja_edit = new ckontrak_kerja_edit();

// Page init
$kontrak_kerja_edit->Page_Init();

// Page main
$kontrak_kerja_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$kontrak_kerja_edit->Page_Render();
?>
...
var CurrentForm = fkontrak_kerjaedit = new ew_Form("fkontrak_kerjaedit", "edit");

// Validate form
fkontrak_kerjaedit.Validate = function() {
...
fkontrak_kerjaedit.Form_CustomValidate = 
...
fkontrak_kerjaedit.ValidateRequired = true;
<?php } else { ?>
fkontrak_kerjaedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$kontrak_kerja_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $kontrak_kerja_edit->ShowPageHeader(); ?>
<?php
$kontrak_kerja_edit->ShowMessage();
?>
<form name="fkontrak_kerjaedit" id="fkontrak_kerjaedit" class="<?php echo $kontrak_kerja_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kontrak_kerja_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kontrak_kerja_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $kontrak_kerja->pegawai_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="kontrak_kerja" data-field="x_pegawai_id" name="x_pegawai_id" id="x_pegawai_id" value="<?php echo ew_HtmlEncode($kontrak_kerja->pegawai_id->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $kontrak_kerja->kontrak_start->EditValue ?></p></span>
</span>
<input type="hidden" data-table="kontrak_kerja" data-field="x_kontrak_start" name="x_kontrak_start" id="x_kontrak_start" value="<?php echo ew_HtmlEncode($kontrak_kerja->kontrak_start->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $kontrak_kerja->kontrak_end->EditValue ?></p></span>
</span>
<input type="hidden" data-table="kontrak_kerja" data-field="x_kontrak_end" name="x_kontrak_end" id="x_kontrak_end" value="<?php echo ew_HtmlEncode($kontrak_kerja->kontrak_end->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kontrak_kerja_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fkontrak_kerjaedit.Init();
</script>
<?php
$kontrak_kerja_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$kontrak_kerja_edit->Page_Terminate();
?>
