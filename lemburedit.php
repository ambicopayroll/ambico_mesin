...
$lembur_edit = NULL; // Initialize page object first

class clembur_edit extends clembur {
	...
	function Page_Main() {
		...
		if ($this->pegawai_id->CurrentValue == "") {
			$this->Page_Terminate("lemburlist.php"); // Invalid key, return to list
		}
		if ($this->lembur_tgl->CurrentValue == "") {
			$this->Page_Terminate("lemburlist.php"); // Invalid key, return to list
		}
		if ($this->lembur_mulai->CurrentValue == "") {
			$this->Page_Terminate("lemburlist.php"); // Invalid key, return to list
		}
		if ($this->lembur_selesai->CurrentValue == "") {
			$this->Page_Terminate("lemburlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->lembur_urut->SetDbValueDef($rsnew, $this->lembur_urut->CurrentValue, 0, $this->lembur_urut->ReadOnly);

			// type_ot
			$this->type_ot->SetDbValueDef($rsnew, $this->type_ot->CurrentValue, 0, $this->type_ot->ReadOnly);

			// lembur_durasi_min
			$this->lembur_durasi_min->SetDbValueDef($rsnew, $this->lembur_durasi_min->CurrentValue, NULL, $this->lembur_durasi_min->ReadOnly);

			// lembur_durasi_max
			$this->lembur_durasi_max->SetDbValueDef($rsnew, $this->lembur_durasi_max->CurrentValue, NULL, $this->lembur_durasi_max->ReadOnly);

			// lembur_keperluan
			$this->lembur_keperluan->SetDbValueDef($rsnew, $this->lembur_keperluan->CurrentValue, NULL, $this->lembur_keperluan->ReadOnly);
...
// Create page object
if (!isset($lembur_edit)) $lembur_edit = new clembur_edit();

// Page init
$lembur_edit->Page_Init();

// Page main
$lembur_edit->Page_Main();
...
var CurrentForm = flemburedit = new ew_Form("flemburedit", "edit");
...
flemburedit.Form_CustomValidate = 
...
flemburedit.ValidateRequired = true;
<?php } else { ?>
flemburedit.ValidateRequired = false; 
<?php } ?>
...
?>
<form name="flemburedit" id="flemburedit" class="<?php echo $lembur_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($lembur_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $lembur_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $lembur->pegawai_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="lembur" data-field="x_pegawai_id" name="x_pegawai_id" id="x_pegawai_id" value="<?php echo ew_HtmlEncode($lembur->pegawai_id->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $lembur->lembur_tgl->EditValue ?></p></span>
</span>
<input type="hidden" data-table="lembur" data-field="x_lembur_tgl" name="x_lembur_tgl" id="x_lembur_tgl" value="<?php echo ew_HtmlEncode($lembur->lembur_tgl->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $lembur->lembur_mulai->EditValue ?></p></span>
</span>
<input type="hidden" data-table="lembur" data-field="x_lembur_mulai" name="x_lembur_mulai" id="x_lembur_mulai" value="<?php echo ew_HtmlEncode($lembur->lembur_mulai->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $lembur->lembur_selesai->EditValue ?></p></span>
</span>
<input type="hidden" data-table="lembur" data-field="x_lembur_selesai" name="x_lembur_selesai" id="x_lembur_selesai" value="<?php echo ew_HtmlEncode($lembur->lembur_selesai->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $lembur_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
