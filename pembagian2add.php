...
$pembagian2_add = NULL; // Initialize page object first

class cpembagian2_add extends cpembagian2 {
	...
	// Page object name
	var $PageObjName = 'pembagian2_add';
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("pembagian2list.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "pembagian2list.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "pembagian2view.php")
...
// Create page object
if (!isset($pembagian2_add)) $pembagian2_add = new cpembagian2_add();

// Page init
$pembagian2_add->Page_Init();

// Page main
$pembagian2_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pembagian2_add->Page_Render();
?>
...
var CurrentForm = fpembagian2add = new ew_Form("fpembagian2add", "add");

// Validate form
fpembagian2add.Validate = function() {
...
fpembagian2add.Form_CustomValidate = 
...
fpembagian2add.ValidateRequired = true;
<?php } else { ?>
fpembagian2add.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $pembagian2_add->ShowPageHeader(); ?>
<?php
$pembagian2_add->ShowMessage();
?>
<form name="fpembagian2add" id="fpembagian2add" class="<?php echo $pembagian2_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pembagian2_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pembagian2_add->Token ?>">
<?php } ?>
...
<?php if ($pembagian2->pembagian2_nama->Visible) { // pembagian2_nama ?>
	<div id="r_pembagian2_nama" class="form-group">
		...
		<div class="col-sm-10"><div<?php echo $pembagian2->pembagian2_nama->CellAttributes() ?>>
...
<?php echo $pembagian2->pembagian2_nama->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($pembagian2->pembagian2_ket->Visible) { // pembagian2_ket ?>
	<div id="r_pembagian2_ket" class="form-group">
		<label id="elh_pembagian2_pembagian2_ket" for="x_pembagian2_ket" class="col-sm-2 control-label ewLabel"><?php echo $pembagian2->pembagian2_ket->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $pembagian2->pembagian2_ket->CellAttributes() ?>>
...
<?php echo $pembagian2->pembagian2_ket->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$pembagian2_add->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $pembagian2_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
