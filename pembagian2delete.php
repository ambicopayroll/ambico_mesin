...
$pembagian2_delete = NULL; // Initialize page object first

class cpembagian2_delete extends cpembagian2 {
	...
	// Page object name
	var $PageObjName = 'pembagian2_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("pembagian2list.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in pembagian2 class, pembagian2info.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("pembagian2list.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($pembagian2_delete)) $pembagian2_delete = new cpembagian2_delete();

// Page init
$pembagian2_delete->Page_Init();

// Page main
$pembagian2_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pembagian2_delete->Page_Render();
?>
...
var CurrentForm = fpembagian2delete = new ew_Form("fpembagian2delete", "delete");

// Form_CustomValidate event
fpembagian2delete.Form_CustomValidate = 
...
fpembagian2delete.ValidateRequired = true;
<?php } else { ?>
fpembagian2delete.ValidateRequired = false; 
<?php } ?>
...
<?php $pembagian2_delete->ShowPageHeader(); ?>
<?php
$pembagian2_delete->ShowMessage();
?>
<form name="fpembagian2delete" id="fpembagian2delete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pembagian2_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pembagian2_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="pembagian2">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($pembagian2_delete->RecKeys as $key) { ?>
...
<?php if ($pembagian2->pembagian2_ket->Visible) { // pembagian2_ket ?>
		<th><span id="elh_pembagian2_pembagian2_ket" class="pembagian2_pembagian2_ket"><?php echo $pembagian2->pembagian2_ket->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
<?php
$pembagian2_delete->RecCnt = 0;
$i = 0;
while (!$pembagian2_delete->Recordset->EOF) {
	$pembagian2_delete->RecCnt++;
	$pembagian2_delete->RowCnt++;

	// Set row properties
	$pembagian2->ResetAttrs();
	$pembagian2->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$pembagian2_delete->LoadRowValues($pembagian2_delete->Recordset);
...
<?php } ?>
<?php if ($pembagian2->pembagian2_ket->Visible) { // pembagian2_ket ?>
		<td<?php echo $pembagian2->pembagian2_ket->CellAttributes() ?>>
<span id="el<?php echo $pembagian2_delete->RowCnt ?>_pembagian2_pembagian2_ket" class="pembagian2_pembagian2_ket">
...
<?php
	$pembagian2_delete->Recordset->MoveNext();
}
$pembagian2_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $pembagian2_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fpembagian2delete.Init();
</script>
<?php
$pembagian2_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$pembagian2_delete->Page_Terminate();
?>
