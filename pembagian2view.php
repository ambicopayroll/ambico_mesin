...
$pembagian2_view = NULL; // Initialize page object first

class cpembagian2_view extends cpembagian2 {
	...
	// Page object name
	var $PageObjName = 'pembagian2_view';
	...
	function __construct() {
		...
		if (@$_GET["pembagian2_id"] <> "") {
			$this->RecKey["pembagian2_id"] = $_GET["pembagian2_id"];
			$KeyUrl .= "&amp;pembagian2_id=" . urlencode($this->RecKey["pembagian2_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["pembagian2_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["pembagian2_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["pembagian2_id"] <> "") {
				...
				$this->RecKey["pembagian2_id"] = $this->pembagian2_id->QueryStringValue;
			} elseif (@$_POST["pembagian2_id"] <> "") {
				$this->pembagian2_id->setFormValue($_POST["pembagian2_id"]);
				$this->RecKey["pembagian2_id"] = $this->pembagian2_id->FormValue;
			} else {
				$sReturnUrl = "pembagian2list.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "pembagian2list.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "pembagian2list.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($pembagian2_view)) $pembagian2_view = new cpembagian2_view();

// Page init
$pembagian2_view->Page_Init();

// Page main
$pembagian2_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pembagian2_view->Page_Render();
?>
...
var CurrentForm = fpembagian2view = new ew_Form("fpembagian2view", "view");

// Form_CustomValidate event
fpembagian2view.Form_CustomValidate = 
...
fpembagian2view.ValidateRequired = true;
<?php } else { ?>
fpembagian2view.ValidateRequired = false; 
<?php } ?>
...
<?php if ($pembagian2->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$pembagian2_view->IsModal) { ?>
<?php if ($pembagian2->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $pembagian2_view->ExportOptions->Render("body") ?>
<?php
	foreach ($pembagian2_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$pembagian2_view->IsModal) { ?>
<?php if ($pembagian2->Export == "") { ?>
...
<?php } ?>
<?php $pembagian2_view->ShowPageHeader(); ?>
<?php
$pembagian2_view->ShowMessage();
?>
<form name="fpembagian2view" id="fpembagian2view" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pembagian2_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pembagian2_view->Token ?>">
<?php } ?>
...
<?php if ($pembagian2->pembagian2_id->Visible) { // pembagian2_id ?>
	<tr id="r_pembagian2_id">
		<td><span id="elh_pembagian2_pembagian2_id"><?php echo $pembagian2->pembagian2_id->FldCaption() ?></span></td>
...
<?php echo $pembagian2->pembagian2_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pembagian2->pembagian2_nama->Visible) { // pembagian2_nama ?>
	<tr id="r_pembagian2_nama">
		<td><span id="elh_pembagian2_pembagian2_nama"><?php echo $pembagian2->pembagian2_nama->FldCaption() ?></span></td>
		<td data-name="pembagian2_nama"<?php echo $pembagian2->pembagian2_nama->CellAttributes() ?>>
<span id="el_pembagian2_pembagian2_nama">
<span<?php echo $pembagian2->pembagian2_nama->ViewAttributes() ?>>
<?php echo $pembagian2->pembagian2_nama->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($pembagian2->pembagian2_ket->Visible) { // pembagian2_ket ?>
	<tr id="r_pembagian2_ket">
		<td><span id="elh_pembagian2_pembagian2_ket"><?php echo $pembagian2->pembagian2_ket->FldCaption() ?></span></td>
		<td data-name="pembagian2_ket"<?php echo $pembagian2->pembagian2_ket->CellAttributes() ?>>
<span id="el_pembagian2_pembagian2_ket">
<span<?php echo $pembagian2->pembagian2_ket->ViewAttributes() ?>>
<?php echo $pembagian2->pembagian2_ket->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($pembagian2->Export == "") { ?>
<script type="text/javascript">
fpembagian2view.Init();
</script>
<?php } ?>
<?php
$pembagian2_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($pembagian2->Export == "") { ?>
...
<?php
$pembagian2_view->Page_Terminate();
?>
