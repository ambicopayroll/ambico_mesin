...
$shift_result_edit = NULL; // Initialize page object first

class cshift_result_edit extends cshift_result {
	...
	// Page object name
	var $PageObjName = 'shift_result_edit';
	...
	function Page_Main() {
		...
		if ($this->pegawai_id->CurrentValue == "") {
			$this->Page_Terminate("shift_resultlist.php"); // Invalid key, return to list
		}
		if ($this->tgl_shift->CurrentValue == "") {
			$this->Page_Terminate("shift_resultlist.php"); // Invalid key, return to list
		}
		if ($this->khusus_lembur->CurrentValue == "") {
			$this->Page_Terminate("shift_resultlist.php"); // Invalid key, return to list
		}
		if ($this->khusus_extra->CurrentValue == "") {
			$this->Page_Terminate("shift_resultlist.php"); // Invalid key, return to list
		}
		if ($this->temp_id_auto->CurrentValue == "") {
			$this->Page_Terminate("shift_resultlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// jns_dok
			$this->jns_dok->SetDbValueDef($rsnew, $this->jns_dok->CurrentValue, 0, $this->jns_dok->ReadOnly);

			// izin_jenis_id
			$this->izin_jenis_id->SetDbValueDef($rsnew, $this->izin_jenis_id->CurrentValue, 0, $this->izin_jenis_id->ReadOnly);

			// cuti_n_id
			$this->cuti_n_id->SetDbValueDef($rsnew, $this->cuti_n_id->CurrentValue, 0, $this->cuti_n_id->ReadOnly);

			// libur_umum
			$this->libur_umum->SetDbValueDef($rsnew, $this->libur_umum->CurrentValue, 0, $this->libur_umum->ReadOnly);

			// libur_rutin
			$this->libur_rutin->SetDbValueDef($rsnew, $this->libur_rutin->CurrentValue, 0, $this->libur_rutin->ReadOnly);

			// jk_ot
			$this->jk_ot->SetDbValueDef($rsnew, $this->jk_ot->CurrentValue, 0, $this->jk_ot->ReadOnly);

			// scan_in
			$this->scan_in->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->scan_in->CurrentValue, 0), ew_CurrentDate(), $this->scan_in->ReadOnly);

			// att_id_in
			$this->att_id_in->SetDbValueDef($rsnew, $this->att_id_in->CurrentValue, "", $this->att_id_in->ReadOnly);

			// late_permission
			$this->late_permission->SetDbValueDef($rsnew, $this->late_permission->CurrentValue, 0, $this->late_permission->ReadOnly);

			// late_minute
			$this->late_minute->SetDbValueDef($rsnew, $this->late_minute->CurrentValue, 0, $this->late_minute->ReadOnly);

			// late
			$this->late->SetDbValueDef($rsnew, $this->late->CurrentValue, 0, $this->late->ReadOnly);

			// break_out
			$this->break_out->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->break_out->CurrentValue, 0), ew_CurrentDate(), $this->break_out->ReadOnly);

			// att_id_break1
			$this->att_id_break1->SetDbValueDef($rsnew, $this->att_id_break1->CurrentValue, "", $this->att_id_break1->ReadOnly);

			// break_in
			$this->break_in->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->break_in->CurrentValue, 0), ew_CurrentDate(), $this->break_in->ReadOnly);

			// att_id_break2
			$this->att_id_break2->SetDbValueDef($rsnew, $this->att_id_break2->CurrentValue, "", $this->att_id_break2->ReadOnly);

			// break_minute
			$this->break_minute->SetDbValueDef($rsnew, $this->break_minute->CurrentValue, 0, $this->break_minute->ReadOnly);

			// break
			$this->break->SetDbValueDef($rsnew, $this->break->CurrentValue, 0, $this->break->ReadOnly);

			// break_ot_minute
			$this->break_ot_minute->SetDbValueDef($rsnew, $this->break_ot_minute->CurrentValue, 0, $this->break_ot_minute->ReadOnly);

			// break_ot
			$this->break_ot->SetDbValueDef($rsnew, $this->break_ot->CurrentValue, 0, $this->break_ot->ReadOnly);

			// early_permission
			$this->early_permission->SetDbValueDef($rsnew, $this->early_permission->CurrentValue, 0, $this->early_permission->ReadOnly);

			// early_minute
			$this->early_minute->SetDbValueDef($rsnew, $this->early_minute->CurrentValue, 0, $this->early_minute->ReadOnly);

			// early
			$this->early->SetDbValueDef($rsnew, $this->early->CurrentValue, 0, $this->early->ReadOnly);

			// scan_out
			$this->scan_out->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->scan_out->CurrentValue, 0), ew_CurrentDate(), $this->scan_out->ReadOnly);

			// att_id_out
			$this->att_id_out->SetDbValueDef($rsnew, $this->att_id_out->CurrentValue, "", $this->att_id_out->ReadOnly);

			// durasi_minute
			$this->durasi_minute->SetDbValueDef($rsnew, $this->durasi_minute->CurrentValue, 0, $this->durasi_minute->ReadOnly);

			// durasi
			$this->durasi->SetDbValueDef($rsnew, $this->durasi->CurrentValue, 0, $this->durasi->ReadOnly);

			// durasi_eot_minute
			$this->durasi_eot_minute->SetDbValueDef($rsnew, $this->durasi_eot_minute->CurrentValue, 0, $this->durasi_eot_minute->ReadOnly);

			// jk_count_as
			$this->jk_count_as->SetDbValueDef($rsnew, $this->jk_count_as->CurrentValue, 0, $this->jk_count_as->ReadOnly);

			// status_jk
			$this->status_jk->SetDbValueDef($rsnew, $this->status_jk->CurrentValue, 0, $this->status_jk->ReadOnly);
...
// Create page object
if (!isset($shift_result_edit)) $shift_result_edit = new cshift_result_edit();

// Page init
$shift_result_edit->Page_Init();

// Page main
$shift_result_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$shift_result_edit->Page_Render();
?>
...
var CurrentForm = fshift_resultedit = new ew_Form("fshift_resultedit", "edit");

// Validate form
fshift_resultedit.Validate = function() {
...
fshift_resultedit.Form_CustomValidate = 
...
fshift_resultedit.ValidateRequired = true;
<?php } else { ?>
fshift_resultedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$shift_result_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $shift_result_edit->ShowPageHeader(); ?>
<?php
$shift_result_edit->ShowMessage();
?>
<form name="fshift_resultedit" id="fshift_resultedit" class="<?php echo $shift_result_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($shift_result_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $shift_result_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $shift_result->pegawai_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="shift_result" data-field="x_pegawai_id" name="x_pegawai_id" id="x_pegawai_id" value="<?php echo ew_HtmlEncode($shift_result->pegawai_id->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $shift_result->tgl_shift->EditValue ?></p></span>
</span>
<input type="hidden" data-table="shift_result" data-field="x_tgl_shift" name="x_tgl_shift" id="x_tgl_shift" value="<?php echo ew_HtmlEncode($shift_result->tgl_shift->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $shift_result->khusus_lembur->EditValue ?></p></span>
</span>
<input type="hidden" data-table="shift_result" data-field="x_khusus_lembur" name="x_khusus_lembur" id="x_khusus_lembur" value="<?php echo ew_HtmlEncode($shift_result->khusus_lembur->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $shift_result->khusus_extra->EditValue ?></p></span>
</span>
<input type="hidden" data-table="shift_result" data-field="x_khusus_extra" name="x_khusus_extra" id="x_khusus_extra" value="<?php echo ew_HtmlEncode($shift_result->khusus_extra->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $shift_result->temp_id_auto->EditValue ?></p></span>
</span>
<input type="hidden" data-table="shift_result" data-field="x_temp_id_auto" name="x_temp_id_auto" id="x_temp_id_auto" value="<?php echo ew_HtmlEncode($shift_result->temp_id_auto->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $shift_result_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fshift_resultedit.Init();
</script>
<?php
$shift_result_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$shift_result_edit->Page_Terminate();
?>
