...
//
// Table class for sms_group
//
class csms_group extends cTable {
	var $group_id;
	var $group_name;
	...
	function __construct() {
		...
		$this->TableVar = 'sms_group';
		$this->TableName = 'sms_group';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`sms_group`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// group_id
		$this->group_id = new cField('sms_group', 'sms_group', 'x_group_id', 'group_id', '`group_id`', '`group_id`', 3, -1, FALSE, '`group_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// group_name
		$this->group_name = new cField('sms_group', 'sms_group', 'x_group_name', 'group_name', '`group_name`', '`group_name`', 200, -1, FALSE, '`group_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`sms_group`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("sms_groupview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("sms_groupview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "sms_groupadd.php?" . $this->UrlParm($parm);
		else
			$url = "sms_groupadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("sms_groupedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("sms_groupadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("sms_groupdelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->group_name->Exportable) $Doc->ExportCaption($this->group_name);
				} else {
					if ($this->group_id->Exportable) $Doc->ExportCaption($this->group_id);
					if ($this->group_name->Exportable) $Doc->ExportCaption($this->group_name);
				}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->group_name->Exportable) $Doc->ExportField($this->group_name);
					} else {
						if ($this->group_id->Exportable) $Doc->ExportField($this->group_id);
						if ($this->group_name->Exportable) $Doc->ExportField($this->group_name);
					}
