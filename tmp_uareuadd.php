...
$tmp_uareu_add = NULL; // Initialize page object first

class ctmp_uareu_add extends ctmp_uareu {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "tmp_uareuview.php")
...
// Create page object
if (!isset($tmp_uareu_add)) $tmp_uareu_add = new ctmp_uareu_add();

// Page init
$tmp_uareu_add->Page_Init();

// Page main
$tmp_uareu_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tmp_uareu_add->Page_Render();
?>
...
var CurrentForm = ftmp_uareuadd = new ew_Form("ftmp_uareuadd", "add");

// Validate form
ftmp_uareuadd.Validate = function() {
...
ftmp_uareuadd.Form_CustomValidate = 
...
ftmp_uareuadd.ValidateRequired = true;
<?php } else { ?>
ftmp_uareuadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $tmp_uareu_add->ShowPageHeader(); ?>
<?php
$tmp_uareu_add->ShowMessage();
?>
<form name="ftmp_uareuadd" id="ftmp_uareuadd" class="<?php echo $tmp_uareu_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tmp_uareu_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tmp_uareu_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="tmp_uareu" data-field="x_pin" name="x_pin" id="x_pin" size="30" maxlength="32" placeholder="<?php echo ew_HtmlEncode($tmp_uareu->pin->getPlaceHolder()) ?>" value="<?php echo $tmp_uareu->pin->EditValue ?>"<?php echo $tmp_uareu->pin->EditAttributes() ?>>
</span>
...
<input type="text" data-table="tmp_uareu" data-field="x_finger_idx" name="x_finger_idx" id="x_finger_idx" size="30" placeholder="<?php echo ew_HtmlEncode($tmp_uareu->finger_idx->getPlaceHolder()) ?>" value="<?php echo $tmp_uareu->finger_idx->EditValue ?>"<?php echo $tmp_uareu->finger_idx->EditAttributes() ?>>
</span>
...
<input type="text" data-table="tmp_uareu" data-field="x_alg_ver" name="x_alg_ver" id="x_alg_ver" size="30" placeholder="<?php echo ew_HtmlEncode($tmp_uareu->alg_ver->getPlaceHolder()) ?>" value="<?php echo $tmp_uareu->alg_ver->EditValue ?>"<?php echo $tmp_uareu->alg_ver->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $tmp_uareu_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
