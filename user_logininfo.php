...
//
class cuser_login extends cTable {
	var $login_id;
	var $login_pwd;
	var $grp_user_id;
	var $lastupdate_date;
	...
	function __construct() {
		...
		// login_id
		$this->login_id = new cField('user_login', 'user_login', 'x_login_id', 'login_id', '`login_id`', '`login_id`', 200, -1, FALSE, '`login_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->login_id->Sortable = TRUE; // Allow sort
		$this->fields['login_id'] = &$this->login_id;

		// login_pwd
		$this->login_pwd = new cField('user_login', 'user_login', 'x_login_pwd', 'login_pwd', '`login_pwd`', '`login_pwd`', 200, -1, FALSE, '`login_pwd`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->login_pwd->Sortable = TRUE; // Allow sort
		$this->fields['login_pwd'] = &$this->login_pwd;

		// grp_user_id
		$this->grp_user_id = new cField('user_login', 'user_login', 'x_grp_user_id', 'grp_user_id', '`grp_user_id`', '`grp_user_id`', 16, -1, FALSE, '`grp_user_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// lastupdate_date
		$this->lastupdate_date = new cField('user_login', 'user_login', 'x_lastupdate_date', 'lastupdate_date', '`lastupdate_date`', ew_CastDateFieldForLike('`lastupdate_date`', 0, "DB"), 135, 0, FALSE, '`lastupdate_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// lastupdate_user
		$this->lastupdate_user = new cField('user_login', 'user_login', 'x_lastupdate_user', 'lastupdate_user', '`lastupdate_user`', '`lastupdate_user`', 200, -1, FALSE, '`lastupdate_user`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('login_id', $rs))
				ew_AddFilter($where, ew_QuotedName('login_id', $this->DBID) . '=' . ew_QuotedValue($rs['login_id'], $this->login_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@login_id@", ew_AdjustSql($this->login_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("user_loginview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("user_loginview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "user_loginadd.php?" . $this->UrlParm($parm);
		else
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("user_loginedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("user_loginadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("user_logindelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "login_id:" . ew_VarToJson($this->login_id->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->login_id->CurrentValue)) {
			$sUrl .= "login_id=" . urlencode($this->login_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["login_id"]))
				$arKeys[] = ew_StripSlashes($_POST["login_id"]);
			elseif (isset($_GET["login_id"]))
				$arKeys[] = ew_StripSlashes($_GET["login_id"]);
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->login_id->CurrentValue = $key;
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->login_pwd->EditValue = $this->login_pwd->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->login_pwd->Exportable) $Doc->ExportCaption($this->login_pwd);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->login_pwd->Exportable) $Doc->ExportField($this->login_pwd);
