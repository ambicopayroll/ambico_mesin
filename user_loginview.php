...
$user_login_view = NULL; // Initialize page object first

class cuser_login_view extends cuser_login {
	...
	// Page object name
	var $PageObjName = 'user_login_view';
	...
	function Page_Init() {
		...
		if (@$_GET["login_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["login_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["login_id"] <> "") {
				...
				$this->login_id->setFormValue($_POST["login_id"]);
				$this->RecKey["login_id"] = $this->login_id->FormValue;
			} else {
				$sReturnUrl = "user_loginlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "user_loginlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "user_loginlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($user_login_view)) $user_login_view = new cuser_login_view();

// Page init
$user_login_view->Page_Init();

// Page main
$user_login_view->Page_Main();
...
var CurrentForm = fuser_loginview = new ew_Form("fuser_loginview", "view");

// Form_CustomValidate event
fuser_loginview.Form_CustomValidate = 
...
fuser_loginview.ValidateRequired = true;
<?php } else { ?>
fuser_loginview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($user_login->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$user_login_view->IsModal) { ?>
<?php if ($user_login->Export == "") { ?>
...
<?php
	foreach ($user_login_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$user_login_view->IsModal) { ?>
<?php if ($user_login->Export == "") { ?>
...
?>
<form name="fuser_loginview" id="fuser_loginview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($user_login_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $user_login_view->Token ?>">
<?php } ?>
...
<?php if ($user_login->login_id->Visible) { // login_id ?>
	<tr id="r_login_id">
		<td><span id="elh_user_login_login_id"><?php echo $user_login->login_id->FldCaption() ?></span></td>
...
<?php echo $user_login->login_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($user_login->login_pwd->Visible) { // login_pwd ?>
	<tr id="r_login_pwd">
		<td><span id="elh_user_login_login_pwd"><?php echo $user_login->login_pwd->FldCaption() ?></span></td>
...
<?php echo $user_login->login_pwd->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($user_login->grp_user_id->Visible) { // grp_user_id ?>
	<tr id="r_grp_user_id">
		<td><span id="elh_user_login_grp_user_id"><?php echo $user_login->grp_user_id->FldCaption() ?></span></td>
...
<?php echo $user_login->grp_user_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($user_login->lastupdate_date->Visible) { // lastupdate_date ?>
	<tr id="r_lastupdate_date">
		<td><span id="elh_user_login_lastupdate_date"><?php echo $user_login->lastupdate_date->FldCaption() ?></span></td>
...
<?php echo $user_login->lastupdate_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($user_login->lastupdate_user->Visible) { // lastupdate_user ?>
	<tr id="r_lastupdate_user">
		<td><span id="elh_user_login_lastupdate_user"><?php echo $user_login->lastupdate_user->FldCaption() ?></span></td>
...
<?php echo $user_login->lastupdate_user->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php
$user_login_view->Page_Terminate();
?>
