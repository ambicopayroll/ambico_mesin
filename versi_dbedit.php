...
$versi_db_edit = NULL; // Initialize page object first

class cversi_db_edit extends cversi_db {
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->no_id->CurrentValue == "") {
			$this->Page_Terminate("versi_dblist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->versi_db->SetDbValueDef($rsnew, $this->versi_db->CurrentValue, "", $this->versi_db->ReadOnly);
...
// Create page object
if (!isset($versi_db_edit)) $versi_db_edit = new cversi_db_edit();

// Page init
$versi_db_edit->Page_Init();

// Page main
$versi_db_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$versi_db_edit->Page_Render();
?>
...
var CurrentForm = fversi_dbedit = new ew_Form("fversi_dbedit", "edit");

// Validate form
fversi_dbedit.Validate = function() {
...
fversi_dbedit.Form_CustomValidate = 
...
fversi_dbedit.ValidateRequired = true;
<?php } else { ?>
fversi_dbedit.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $versi_db_edit->ShowPageHeader(); ?>
<?php
$versi_db_edit->ShowMessage();
?>
<form name="fversi_dbedit" id="fversi_dbedit" class="<?php echo $versi_db_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($versi_db_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $versi_db_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $versi_db->no_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="versi_db" data-field="x_no_id" name="x_no_id" id="x_no_id" value="<?php echo ew_HtmlEncode($versi_db->no_id->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $versi_db_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
