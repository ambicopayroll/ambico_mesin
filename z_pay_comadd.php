...
$z_pay_com_add = NULL; // Initialize page object first

class cz_pay_com_add extends cz_pay_com {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "z_pay_comview.php")
	...
	function LoadDefaultValues() {
		...
		$this->com_kode->CurrentValue = NULL;
		$this->com_kode->OldValue = $this->com_kode->CurrentValue;
		$this->com_name->CurrentValue = NULL;
		$this->com_name->OldValue = $this->com_name->CurrentValue;
		$this->type_com->CurrentValue = 0;
		$this->fluctuate->CurrentValue = 0;
		$this->no_urut->CurrentValue = NULL;
		$this->no_urut->OldValue = $this->no_urut->CurrentValue;
		$this->param->CurrentValue = 0;
		$this->hitung->CurrentValue = 1;
		$this->dibayar->CurrentValue = 2;
		$this->cara_bayar->CurrentValue = 1;
		$this->pinjaman->CurrentValue = 0;
	...
	function AddRow($rsold = NULL) {
		...
		// com_kode
		$this->com_kode->SetDbValueDef($rsnew, $this->com_kode->CurrentValue, "", FALSE);

		// com_name
		$this->com_name->SetDbValueDef($rsnew, $this->com_name->CurrentValue, "", FALSE);

		// type_com
		$this->type_com->SetDbValueDef($rsnew, $this->type_com->CurrentValue, 0, strval($this->type_com->CurrentValue) == "");

		// fluctuate
		$this->fluctuate->SetDbValueDef($rsnew, $this->fluctuate->CurrentValue, 0, strval($this->fluctuate->CurrentValue) == "");

		// no_urut
		$this->no_urut->SetDbValueDef($rsnew, $this->no_urut->CurrentValue, 0, FALSE);

		// param
		$this->param->SetDbValueDef($rsnew, $this->param->CurrentValue, 0, strval($this->param->CurrentValue) == "");
		...
		// cara_bayar
		$this->cara_bayar->SetDbValueDef($rsnew, $this->cara_bayar->CurrentValue, 0, strval($this->cara_bayar->CurrentValue) == "");

		// pinjaman
		$this->pinjaman->SetDbValueDef($rsnew, $this->pinjaman->CurrentValue, 0, strval($this->pinjaman->CurrentValue) == "");
...
// Create page object
if (!isset($z_pay_com_add)) $z_pay_com_add = new cz_pay_com_add();

// Page init
$z_pay_com_add->Page_Init();

// Page main
$z_pay_com_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$z_pay_com_add->Page_Render();
?>
...
var CurrentForm = fz_pay_comadd = new ew_Form("fz_pay_comadd", "add");

// Validate form
fz_pay_comadd.Validate = function() {
...
fz_pay_comadd.Form_CustomValidate = 
...
fz_pay_comadd.ValidateRequired = true;
<?php } else { ?>
fz_pay_comadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $z_pay_com_add->ShowPageHeader(); ?>
<?php
$z_pay_com_add->ShowMessage();
?>
<form name="fz_pay_comadd" id="fz_pay_comadd" class="<?php echo $z_pay_com_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($z_pay_com_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $z_pay_com_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="z_pay_com" data-field="x_com_id" name="x_com_id" id="x_com_id" size="30" placeholder="<?php echo ew_HtmlEncode($z_pay_com->com_id->getPlaceHolder()) ?>" value="<?php echo $z_pay_com->com_id->EditValue ?>"<?php echo $z_pay_com->com_id->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $z_pay_com_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
