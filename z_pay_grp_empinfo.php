...
//
// Table class for z_pay_grp_emp
//
class cz_pay_grp_emp extends cTable {
	var $grp_id;
	var $emp_id_auto;
	var $no_rek;
	...
	function __construct() {
		...
		$this->TableVar = 'z_pay_grp_emp';
		$this->TableName = 'z_pay_grp_emp';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`z_pay_grp_emp`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// grp_id
		$this->grp_id = new cField('z_pay_grp_emp', 'z_pay_grp_emp', 'x_grp_id', 'grp_id', '`grp_id`', '`grp_id`', 2, -1, FALSE, '`grp_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// emp_id_auto
		$this->emp_id_auto = new cField('z_pay_grp_emp', 'z_pay_grp_emp', 'x_emp_id_auto', 'emp_id_auto', '`emp_id_auto`', '`emp_id_auto`', 3, -1, FALSE, '`emp_id_auto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// no_rek
		$this->no_rek = new cField('z_pay_grp_emp', 'z_pay_grp_emp', 'x_no_rek', 'no_rek', '`no_rek`', '`no_rek`', 200, -1, FALSE, '`no_rek`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`z_pay_grp_emp`";
	}
	...
	function SqlKeyFilter() {
		return "`grp_id` = @grp_id@ AND `emp_id_auto` = @emp_id_auto@";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("z_pay_grp_empview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("z_pay_grp_empview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "z_pay_grp_empadd.php?" . $this->UrlParm($parm);
		else
			$url = "z_pay_grp_empadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("z_pay_grp_empedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("z_pay_grp_empadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("z_pay_grp_empdelete.php", $this->UrlParm());
	}
		...
		if (is_array($arKeys)) {
			foreach ($arKeys as $key) {
				...
				if (!is_numeric($key[0])) // grp_id
					continue;
				if (!is_numeric($key[1])) // emp_id_auto
					continue;
				$ar[] = $key;
			}
		}
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->emp_id_auto->CurrentValue = $key[1];
