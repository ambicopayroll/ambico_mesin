...
$z_pay_grp_add = NULL; // Initialize page object first

class cz_pay_grp_add extends cz_pay_grp {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "z_pay_grpview.php")
	...
	function LoadDefaultValues() {
		...
		$this->kode_grp->CurrentValue = NULL;
		$this->kode_grp->OldValue = $this->kode_grp->CurrentValue;
		$this->grp_name->CurrentValue = NULL;
		$this->grp_name->OldValue = $this->grp_name->CurrentValue;
		$this->use_pengurang->CurrentValue = 0;
		$this->type_pengurang->CurrentValue = 0;
		$this->pengurang_persen->CurrentValue = 0;
	...
	function AddRow($rsold = NULL) {
		...
		// kode_grp
		$this->kode_grp->SetDbValueDef($rsnew, $this->kode_grp->CurrentValue, "", FALSE);

		// grp_name
		$this->grp_name->SetDbValueDef($rsnew, $this->grp_name->CurrentValue, "", FALSE);

		// use_pengurang
		$this->use_pengurang->SetDbValueDef($rsnew, $this->use_pengurang->CurrentValue, 0, strval($this->use_pengurang->CurrentValue) == "");

		// type_pengurang
		$this->type_pengurang->SetDbValueDef($rsnew, $this->type_pengurang->CurrentValue, 0, strval($this->type_pengurang->CurrentValue) == "");

		// pengurang_persen
		$this->pengurang_persen->SetDbValueDef($rsnew, $this->pengurang_persen->CurrentValue, 0, strval($this->pengurang_persen->CurrentValue) == "");
...
// Create page object
if (!isset($z_pay_grp_add)) $z_pay_grp_add = new cz_pay_grp_add();

// Page init
$z_pay_grp_add->Page_Init();

// Page main
$z_pay_grp_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$z_pay_grp_add->Page_Render();
?>
...
var CurrentForm = fz_pay_grpadd = new ew_Form("fz_pay_grpadd", "add");

// Validate form
fz_pay_grpadd.Validate = function() {
...
fz_pay_grpadd.Form_CustomValidate = 
...
fz_pay_grpadd.ValidateRequired = true;
<?php } else { ?>
fz_pay_grpadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $z_pay_grp_add->ShowPageHeader(); ?>
<?php
$z_pay_grp_add->ShowMessage();
?>
<form name="fz_pay_grpadd" id="fz_pay_grpadd" class="<?php echo $z_pay_grp_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($z_pay_grp_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $z_pay_grp_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="z_pay_grp" data-field="x_grp_id" name="x_grp_id" id="x_grp_id" size="30" placeholder="<?php echo ew_HtmlEncode($z_pay_grp->grp_id->getPlaceHolder()) ?>" value="<?php echo $z_pay_grp->grp_id->EditValue ?>"<?php echo $z_pay_grp->grp_id->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $z_pay_grp_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
