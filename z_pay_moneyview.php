...
$z_pay_money_view = NULL; // Initialize page object first

class cz_pay_money_view extends cz_pay_money {
	...
	// Page object name
	var $PageObjName = 'z_pay_money_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["com_id"] <> "") {
				...
				$sReturnUrl = "z_pay_moneylist.php"; // Return to list
			}
			if (@$_GET["grp_id"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "z_pay_moneylist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "z_pay_moneylist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($z_pay_money_view)) $z_pay_money_view = new cz_pay_money_view();

// Page init
$z_pay_money_view->Page_Init();

// Page main
$z_pay_money_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$z_pay_money_view->Page_Render();
?>
...
var CurrentForm = fz_pay_moneyview = new ew_Form("fz_pay_moneyview", "view");

// Form_CustomValidate event
fz_pay_moneyview.Form_CustomValidate = 
...
fz_pay_moneyview.ValidateRequired = true;
<?php } else { ?>
fz_pay_moneyview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($z_pay_money->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$z_pay_money_view->IsModal) { ?>
<?php if ($z_pay_money->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $z_pay_money_view->ExportOptions->Render("body") ?>
<?php
	foreach ($z_pay_money_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$z_pay_money_view->IsModal) { ?>
<?php if ($z_pay_money->Export == "") { ?>
...
<?php } ?>
<?php $z_pay_money_view->ShowPageHeader(); ?>
<?php
$z_pay_money_view->ShowMessage();
?>
<form name="fz_pay_moneyview" id="fz_pay_moneyview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($z_pay_money_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $z_pay_money_view->Token ?>">
<?php } ?>
...
<?php if ($z_pay_money->com_id->Visible) { // com_id ?>
	<tr id="r_com_id">
		<td><span id="elh_z_pay_money_com_id"><?php echo $z_pay_money->com_id->FldCaption() ?></span></td>
...
<?php echo $z_pay_money->com_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_money->grp_id->Visible) { // grp_id ?>
	<tr id="r_grp_id">
		<td><span id="elh_z_pay_money_grp_id"><?php echo $z_pay_money->grp_id->FldCaption() ?></span></td>
...
<?php echo $z_pay_money->grp_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_money->emp_id_auto->Visible) { // emp_id_auto ?>
	<tr id="r_emp_id_auto">
		<td><span id="elh_z_pay_money_emp_id_auto"><?php echo $z_pay_money->emp_id_auto->FldCaption() ?></span></td>
...
<?php echo $z_pay_money->emp_id_auto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_money->nilai_rp->Visible) { // nilai_rp ?>
	<tr id="r_nilai_rp">
		<td><span id="elh_z_pay_money_nilai_rp"><?php echo $z_pay_money->nilai_rp->FldCaption() ?></span></td>
...
<?php echo $z_pay_money->nilai_rp->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_money->lastupdate_date->Visible) { // lastupdate_date ?>
	<tr id="r_lastupdate_date">
		<td><span id="elh_z_pay_money_lastupdate_date"><?php echo $z_pay_money->lastupdate_date->FldCaption() ?></span></td>
...
<?php echo $z_pay_money->lastupdate_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_money->lastupdate_user->Visible) { // lastupdate_user ?>
	<tr id="r_lastupdate_user">
		<td><span id="elh_z_pay_money_lastupdate_user"><?php echo $z_pay_money->lastupdate_user->FldCaption() ?></span></td>
...
<?php echo $z_pay_money->lastupdate_user->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($z_pay_money->Export == "") { ?>
<script type="text/javascript">
fz_pay_moneyview.Init();
</script>
<?php } ?>
<?php
$z_pay_money_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($z_pay_money->Export == "") { ?>
...
<?php
$z_pay_money_view->Page_Terminate();
?>
