...
$z_pay_process_d_view = NULL; // Initialize page object first

class cz_pay_process_d_view extends cz_pay_process_d {
	...
	// Page object name
	var $PageObjName = 'z_pay_process_d_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["process_id"] <> "") {
				...
				$sReturnUrl = "z_pay_process_dlist.php"; // Return to list
			}
			if (@$_GET["no_urut"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "z_pay_process_dlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "z_pay_process_dlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($z_pay_process_d_view)) $z_pay_process_d_view = new cz_pay_process_d_view();

// Page init
$z_pay_process_d_view->Page_Init();

// Page main
$z_pay_process_d_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$z_pay_process_d_view->Page_Render();
?>
...
var CurrentForm = fz_pay_process_dview = new ew_Form("fz_pay_process_dview", "view");

// Form_CustomValidate event
fz_pay_process_dview.Form_CustomValidate = 
...
fz_pay_process_dview.ValidateRequired = true;
<?php } else { ?>
fz_pay_process_dview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($z_pay_process_d->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$z_pay_process_d_view->IsModal) { ?>
<?php if ($z_pay_process_d->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $z_pay_process_d_view->ExportOptions->Render("body") ?>
<?php
	foreach ($z_pay_process_d_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$z_pay_process_d_view->IsModal) { ?>
<?php if ($z_pay_process_d->Export == "") { ?>
...
<?php } ?>
<?php $z_pay_process_d_view->ShowPageHeader(); ?>
<?php
$z_pay_process_d_view->ShowMessage();
?>
<form name="fz_pay_process_dview" id="fz_pay_process_dview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($z_pay_process_d_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $z_pay_process_d_view->Token ?>">
<?php } ?>
...
<?php if ($z_pay_process_d->process_id->Visible) { // process_id ?>
	<tr id="r_process_id">
		<td><span id="elh_z_pay_process_d_process_id"><?php echo $z_pay_process_d->process_id->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_d->process_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_d->no_urut->Visible) { // no_urut ?>
	<tr id="r_no_urut">
		<td><span id="elh_z_pay_process_d_no_urut"><?php echo $z_pay_process_d->no_urut->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_d->no_urut->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_d->emp_id_auto->Visible) { // emp_id_auto ?>
	<tr id="r_emp_id_auto">
		<td><span id="elh_z_pay_process_d_emp_id_auto"><?php echo $z_pay_process_d->emp_id_auto->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_d->emp_id_auto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_d->tot_payroll->Visible) { // tot_payroll ?>
	<tr id="r_tot_payroll">
		<td><span id="elh_z_pay_process_d_tot_payroll"><?php echo $z_pay_process_d->tot_payroll->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_d->tot_payroll->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($z_pay_process_d->Export == "") { ?>
<script type="text/javascript">
fz_pay_process_dview.Init();
</script>
<?php } ?>
<?php
$z_pay_process_d_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($z_pay_process_d->Export == "") { ?>
...
<?php
$z_pay_process_d_view->Page_Terminate();
?>
