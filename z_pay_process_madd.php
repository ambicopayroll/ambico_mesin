...
$z_pay_process_m_add = NULL; // Initialize page object first

class cz_pay_process_m_add extends cz_pay_process_m {
	...
	// Page object name
	var $PageObjName = 'z_pay_process_m_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "z_pay_process_mview.php")
	...
	function LoadDefaultValues() {
		...
		$this->process_name->CurrentValue = NULL;
		$this->process_name->OldValue = $this->process_name->CurrentValue;
		...
		$this->round_value->CurrentValue = NULL;
		$this->round_value->OldValue = $this->round_value->CurrentValue;
		$this->tot_process->CurrentValue = NULL;
		$this->tot_process->OldValue = $this->tot_process->CurrentValue;
		$this->create_by->CurrentValue = NULL;
		$this->create_by->OldValue = $this->create_by->CurrentValue;
		$this->check_by->CurrentValue = NULL;
		$this->check_by->OldValue = $this->check_by->CurrentValue;
		$this->approve_by->CurrentValue = NULL;
		$this->approve_by->OldValue = $this->approve_by->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// process_name
		$this->process_name->SetDbValueDef($rsnew, $this->process_name->CurrentValue, "", FALSE);

		// date1
		$this->date1->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->date1->CurrentValue, 0), ew_CurrentDate(), FALSE);

		// date2
		$this->date2->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->date2->CurrentValue, 0), ew_CurrentDate(), FALSE);

		// payment_date
		$this->payment_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->payment_date->CurrentValue, 0), NULL, FALSE);

		// round_value
		$this->round_value->SetDbValueDef($rsnew, $this->round_value->CurrentValue, 0, FALSE);

		// tot_process
		$this->tot_process->SetDbValueDef($rsnew, $this->tot_process->CurrentValue, 0, FALSE);

		// create_by
		$this->create_by->SetDbValueDef($rsnew, $this->create_by->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($z_pay_process_m_add)) $z_pay_process_m_add = new cz_pay_process_m_add();

// Page init
$z_pay_process_m_add->Page_Init();

// Page main
$z_pay_process_m_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$z_pay_process_m_add->Page_Render();
?>
...
var CurrentForm = fz_pay_process_madd = new ew_Form("fz_pay_process_madd", "add");

// Validate form
fz_pay_process_madd.Validate = function() {
...
fz_pay_process_madd.Form_CustomValidate = 
...
fz_pay_process_madd.ValidateRequired = true;
<?php } else { ?>
fz_pay_process_madd.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$z_pay_process_m_add->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $z_pay_process_m_add->ShowPageHeader(); ?>
<?php
$z_pay_process_m_add->ShowMessage();
?>
<form name="fz_pay_process_madd" id="fz_pay_process_madd" class="<?php echo $z_pay_process_m_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($z_pay_process_m_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $z_pay_process_m_add->Token ?>">
<?php } ?>
...
<input type="text" data-table="z_pay_process_m" data-field="x_process_id" name="x_process_id" id="x_process_id" size="30" placeholder="<?php echo ew_HtmlEncode($z_pay_process_m->process_id->getPlaceHolder()) ?>" value="<?php echo $z_pay_process_m->process_id->EditValue ?>"<?php echo $z_pay_process_m->process_id->EditAttributes() ?>>
</span>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $z_pay_process_m_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fz_pay_process_madd.Init();
</script>
<?php
$z_pay_process_m_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$z_pay_process_m_add->Page_Terminate();
?>
