...
$z_pay_process_sd_edit = NULL; // Initialize page object first

class cz_pay_process_sd_edit extends cz_pay_process_sd {
	...
	// Page object name
	var $PageObjName = 'z_pay_process_sd_edit';
	...
	function Page_Main() {
		...
		if ($this->process_id->CurrentValue == "") {
			$this->Page_Terminate("z_pay_process_sdlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// rumus
			$this->rumus->SetDbValueDef($rsnew, $this->rumus->CurrentValue, "", $this->rumus->ReadOnly);

			// subtot_payroll
			$this->subtot_payroll->SetDbValueDef($rsnew, $this->subtot_payroll->CurrentValue, 0, $this->subtot_payroll->ReadOnly);

			// jml_faktor
			$this->jml_faktor->SetDbValueDef($rsnew, $this->jml_faktor->CurrentValue, 0, $this->jml_faktor->ReadOnly);
...
// Create page object
if (!isset($z_pay_process_sd_edit)) $z_pay_process_sd_edit = new cz_pay_process_sd_edit();

// Page init
$z_pay_process_sd_edit->Page_Init();

// Page main
$z_pay_process_sd_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$z_pay_process_sd_edit->Page_Render();
?>
...
var CurrentForm = fz_pay_process_sdedit = new ew_Form("fz_pay_process_sdedit", "edit");

// Validate form
fz_pay_process_sdedit.Validate = function() {
...
fz_pay_process_sdedit.Form_CustomValidate = 
...
fz_pay_process_sdedit.ValidateRequired = true;
<?php } else { ?>
fz_pay_process_sdedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$z_pay_process_sd_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $z_pay_process_sd_edit->ShowPageHeader(); ?>
<?php
$z_pay_process_sd_edit->ShowMessage();
?>
<form name="fz_pay_process_sdedit" id="fz_pay_process_sdedit" class="<?php echo $z_pay_process_sd_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($z_pay_process_sd_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $z_pay_process_sd_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $z_pay_process_sd->process_id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="z_pay_process_sd" data-field="x_process_id" name="x_process_id" id="x_process_id" value="<?php echo ew_HtmlEncode($z_pay_process_sd->process_id->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $z_pay_process_sd->no_urut->EditValue ?></p></span>
</span>
<input type="hidden" data-table="z_pay_process_sd" data-field="x_no_urut" name="x_no_urut" id="x_no_urut" value="<?php echo ew_HtmlEncode($z_pay_process_sd->no_urut->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $z_pay_process_sd->no_urut_ref->EditValue ?></p></span>
</span>
<input type="hidden" data-table="z_pay_process_sd" data-field="x_no_urut_ref" name="x_no_urut_ref" id="x_no_urut_ref" value="<?php echo ew_HtmlEncode($z_pay_process_sd->no_urut_ref->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $z_pay_process_sd_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fz_pay_process_sdedit.Init();
</script>
<?php
$z_pay_process_sd_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$z_pay_process_sd_edit->Page_Terminate();
?>
