...
$z_pay_process_sd_view = NULL; // Initialize page object first

class cz_pay_process_sd_view extends cz_pay_process_sd {
	...
	// Page object name
	var $PageObjName = 'z_pay_process_sd_view';
	...
	function __construct() {
		...
		if (@$_GET["no_urut"] <> "") {
			$this->RecKey["no_urut"] = $_GET["no_urut"];
			$KeyUrl .= "&amp;no_urut=" . urlencode($this->RecKey["no_urut"]);
		}
		if (@$_GET["no_urut_ref"] <> "") {
	...
	function Page_Init() {
		...
		if (@$_GET["no_urut"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
			$gsExportFile .= ew_StripSlashes($_GET["no_urut"]);
		}
		if (@$_GET["no_urut_ref"] <> "") {
			if ($gsExportFile <> "") $gsExportFile .= "_";
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["process_id"] <> "") {
				...
				$sReturnUrl = "z_pay_process_sdlist.php"; // Return to list
			}
			if (@$_GET["no_urut"] <> "") {
				$this->no_urut->setQueryStringValue($_GET["no_urut"]);
				$this->RecKey["no_urut"] = $this->no_urut->QueryStringValue;
			} elseif (@$_POST["no_urut"] <> "") {
				$this->no_urut->setFormValue($_POST["no_urut"]);
				$this->RecKey["no_urut"] = $this->no_urut->FormValue;
			} else {
				$sReturnUrl = "z_pay_process_sdlist.php"; // Return to list
			}
			if (@$_GET["no_urut_ref"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "z_pay_process_sdlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "z_pay_process_sdlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($z_pay_process_sd_view)) $z_pay_process_sd_view = new cz_pay_process_sd_view();

// Page init
$z_pay_process_sd_view->Page_Init();

// Page main
$z_pay_process_sd_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$z_pay_process_sd_view->Page_Render();
?>
...
var CurrentForm = fz_pay_process_sdview = new ew_Form("fz_pay_process_sdview", "view");

// Form_CustomValidate event
fz_pay_process_sdview.Form_CustomValidate = 
...
fz_pay_process_sdview.ValidateRequired = true;
<?php } else { ?>
fz_pay_process_sdview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($z_pay_process_sd->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$z_pay_process_sd_view->IsModal) { ?>
<?php if ($z_pay_process_sd->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $z_pay_process_sd_view->ExportOptions->Render("body") ?>
<?php
	foreach ($z_pay_process_sd_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$z_pay_process_sd_view->IsModal) { ?>
<?php if ($z_pay_process_sd->Export == "") { ?>
...
<?php } ?>
<?php $z_pay_process_sd_view->ShowPageHeader(); ?>
<?php
$z_pay_process_sd_view->ShowMessage();
?>
<form name="fz_pay_process_sdview" id="fz_pay_process_sdview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($z_pay_process_sd_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $z_pay_process_sd_view->Token ?>">
<?php } ?>
...
<?php if ($z_pay_process_sd->process_id->Visible) { // process_id ?>
	<tr id="r_process_id">
		<td><span id="elh_z_pay_process_sd_process_id"><?php echo $z_pay_process_sd->process_id->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->process_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_sd->no_urut->Visible) { // no_urut ?>
	<tr id="r_no_urut">
		<td><span id="elh_z_pay_process_sd_no_urut"><?php echo $z_pay_process_sd->no_urut->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->no_urut->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_sd->no_urut_ref->Visible) { // no_urut_ref ?>
	<tr id="r_no_urut_ref">
		<td><span id="elh_z_pay_process_sd_no_urut_ref"><?php echo $z_pay_process_sd->no_urut_ref->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->no_urut_ref->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_sd->emp_id_auto->Visible) { // emp_id_auto ?>
	<tr id="r_emp_id_auto">
		<td><span id="elh_z_pay_process_sd_emp_id_auto"><?php echo $z_pay_process_sd->emp_id_auto->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->emp_id_auto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_sd->com_id->Visible) { // com_id ?>
	<tr id="r_com_id">
		<td><span id="elh_z_pay_process_sd_com_id"><?php echo $z_pay_process_sd->com_id->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->com_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_sd->kondisi->Visible) { // kondisi ?>
	<tr id="r_kondisi">
		<td><span id="elh_z_pay_process_sd_kondisi"><?php echo $z_pay_process_sd->kondisi->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->kondisi->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_sd->rumus->Visible) { // rumus ?>
	<tr id="r_rumus">
		<td><span id="elh_z_pay_process_sd_rumus"><?php echo $z_pay_process_sd->rumus->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->rumus->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_sd->subtot_payroll->Visible) { // subtot_payroll ?>
	<tr id="r_subtot_payroll">
		<td><span id="elh_z_pay_process_sd_subtot_payroll"><?php echo $z_pay_process_sd->subtot_payroll->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->subtot_payroll->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($z_pay_process_sd->jml_faktor->Visible) { // jml_faktor ?>
	<tr id="r_jml_faktor">
		<td><span id="elh_z_pay_process_sd_jml_faktor"><?php echo $z_pay_process_sd->jml_faktor->FldCaption() ?></span></td>
...
<?php echo $z_pay_process_sd->jml_faktor->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($z_pay_process_sd->Export == "") { ?>
<script type="text/javascript">
fz_pay_process_sdview.Init();
</script>
<?php } ?>
<?php
$z_pay_process_sd_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($z_pay_process_sd->Export == "") { ?>
...
<?php
$z_pay_process_sd_view->Page_Terminate();
?>
