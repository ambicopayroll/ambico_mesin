...
$z_pay_report_edit = NULL; // Initialize page object first

class cz_pay_report_edit extends cz_pay_report {
	...
	// Page object name
	var $PageObjName = 'z_pay_report_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->id_kode_report->CurrentValue == "") {
			$this->Page_Terminate("z_pay_reportlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->report_code->SetDbValueDef($rsnew, $this->report_code->CurrentValue, "", $this->report_code->ReadOnly);

			// report_name
			$this->report_name->SetDbValueDef($rsnew, $this->report_name->CurrentValue, "", $this->report_name->ReadOnly);
...
// Create page object
if (!isset($z_pay_report_edit)) $z_pay_report_edit = new cz_pay_report_edit();

// Page init
$z_pay_report_edit->Page_Init();

// Page main
$z_pay_report_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$z_pay_report_edit->Page_Render();
?>
...
var CurrentForm = fz_pay_reportedit = new ew_Form("fz_pay_reportedit", "edit");

// Validate form
fz_pay_reportedit.Validate = function() {
...
fz_pay_reportedit.Form_CustomValidate = 
...
fz_pay_reportedit.ValidateRequired = true;
<?php } else { ?>
fz_pay_reportedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$z_pay_report_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $z_pay_report_edit->ShowPageHeader(); ?>
<?php
$z_pay_report_edit->ShowMessage();
?>
<form name="fz_pay_reportedit" id="fz_pay_reportedit" class="<?php echo $z_pay_report_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($z_pay_report_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $z_pay_report_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $z_pay_report->id_kode_report->EditValue ?></p></span>
</span>
<input type="hidden" data-table="z_pay_report" data-field="x_id_kode_report" name="x_id_kode_report" id="x_id_kode_report" value="<?php echo ew_HtmlEncode($z_pay_report->id_kode_report->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $z_pay_report_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fz_pay_reportedit.Init();
</script>
<?php
$z_pay_report_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$z_pay_report_edit->Page_Terminate();
?>
