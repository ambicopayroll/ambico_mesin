...
$zx_bayar_kredit_view = NULL; // Initialize page object first

class czx_bayar_kredit_view extends czx_bayar_kredit {
	...
	// Page object name
	var $PageObjName = 'zx_bayar_kredit_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["id_bayar"] <> "") {
			$this->RecKey["id_bayar"] = $_GET["id_bayar"];
			$KeyUrl .= "&amp;id_bayar=" . urlencode($this->RecKey["id_bayar"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["id_bayar"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["id_bayar"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_bayar"] <> "") {
				...
				$this->RecKey["id_bayar"] = $this->id_bayar->QueryStringValue;
			} elseif (@$_POST["id_bayar"] <> "") {
				$this->id_bayar->setFormValue($_POST["id_bayar"]);
				$this->RecKey["id_bayar"] = $this->id_bayar->FormValue;
			} else {
				$sReturnUrl = "zx_bayar_kreditlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "zx_bayar_kreditlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "zx_bayar_kreditlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($zx_bayar_kredit_view)) $zx_bayar_kredit_view = new czx_bayar_kredit_view();

// Page init
$zx_bayar_kredit_view->Page_Init();

// Page main
$zx_bayar_kredit_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$zx_bayar_kredit_view->Page_Render();
?>
...
var CurrentForm = fzx_bayar_kreditview = new ew_Form("fzx_bayar_kreditview", "view");

// Form_CustomValidate event
fzx_bayar_kreditview.Form_CustomValidate = 
...
fzx_bayar_kreditview.ValidateRequired = true;
<?php } else { ?>
fzx_bayar_kreditview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($zx_bayar_kredit->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$zx_bayar_kredit_view->IsModal) { ?>
<?php if ($zx_bayar_kredit->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $zx_bayar_kredit_view->ExportOptions->Render("body") ?>
<?php
	foreach ($zx_bayar_kredit_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$zx_bayar_kredit_view->IsModal) { ?>
<?php if ($zx_bayar_kredit->Export == "") { ?>
...
<?php } ?>
<?php $zx_bayar_kredit_view->ShowPageHeader(); ?>
<?php
$zx_bayar_kredit_view->ShowMessage();
?>
<form name="fzx_bayar_kreditview" id="fzx_bayar_kreditview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($zx_bayar_kredit_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $zx_bayar_kredit_view->Token ?>">
<?php } ?>
...
<?php if ($zx_bayar_kredit->id_bayar->Visible) { // id_bayar ?>
	<tr id="r_id_bayar">
		<td><span id="elh_zx_bayar_kredit_id_bayar"><?php echo $zx_bayar_kredit->id_bayar->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->id_bayar->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->tgl_bayar->Visible) { // tgl_bayar ?>
	<tr id="r_tgl_bayar">
		<td><span id="elh_zx_bayar_kredit_tgl_bayar"><?php echo $zx_bayar_kredit->tgl_bayar->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->tgl_bayar->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->id_kredit->Visible) { // id_kredit ?>
	<tr id="r_id_kredit">
		<td><span id="elh_zx_bayar_kredit_id_kredit"><?php echo $zx_bayar_kredit->id_kredit->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->id_kredit->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->no_urut->Visible) { // no_urut ?>
	<tr id="r_no_urut">
		<td><span id="elh_zx_bayar_kredit_no_urut"><?php echo $zx_bayar_kredit->no_urut->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->no_urut->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->tgl_jt->Visible) { // tgl_jt ?>
	<tr id="r_tgl_jt">
		<td><span id="elh_zx_bayar_kredit_tgl_jt"><?php echo $zx_bayar_kredit->tgl_jt->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->tgl_jt->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->debet->Visible) { // debet ?>
	<tr id="r_debet">
		<td><span id="elh_zx_bayar_kredit_debet"><?php echo $zx_bayar_kredit->debet->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->debet->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->angs_pokok->Visible) { // angs_pokok ?>
	<tr id="r_angs_pokok">
		<td><span id="elh_zx_bayar_kredit_angs_pokok"><?php echo $zx_bayar_kredit->angs_pokok->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->angs_pokok->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->bunga->Visible) { // bunga ?>
	<tr id="r_bunga">
		<td><span id="elh_zx_bayar_kredit_bunga"><?php echo $zx_bayar_kredit->bunga->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->bunga->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->emp_id_auto->Visible) { // emp_id_auto ?>
	<tr id="r_emp_id_auto">
		<td><span id="elh_zx_bayar_kredit_emp_id_auto"><?php echo $zx_bayar_kredit->emp_id_auto->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->emp_id_auto->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->keterangan->Visible) { // keterangan ?>
	<tr id="r_keterangan">
		<td><span id="elh_zx_bayar_kredit_keterangan"><?php echo $zx_bayar_kredit->keterangan->FldCaption() ?></span></td>
		<td data-name="keterangan"<?php echo $zx_bayar_kredit->keterangan->CellAttributes() ?>>
<span id="el_zx_bayar_kredit_keterangan">
<span<?php echo $zx_bayar_kredit->keterangan->ViewAttributes() ?>>
<?php echo $zx_bayar_kredit->keterangan->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->status->Visible) { // status ?>
	<tr id="r_status">
		<td><span id="elh_zx_bayar_kredit_status"><?php echo $zx_bayar_kredit->status->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->status->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->lastupdate_date->Visible) { // lastupdate_date ?>
	<tr id="r_lastupdate_date">
		<td><span id="elh_zx_bayar_kredit_lastupdate_date"><?php echo $zx_bayar_kredit->lastupdate_date->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->lastupdate_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_bayar_kredit->lastupdate_user->Visible) { // lastupdate_user ?>
	<tr id="r_lastupdate_user">
		<td><span id="elh_zx_bayar_kredit_lastupdate_user"><?php echo $zx_bayar_kredit->lastupdate_user->FldCaption() ?></span></td>
...
<?php echo $zx_bayar_kredit->lastupdate_user->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($zx_bayar_kredit->Export == "") { ?>
<script type="text/javascript">
fzx_bayar_kreditview.Init();
</script>
<?php } ?>
<?php
$zx_bayar_kredit_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($zx_bayar_kredit->Export == "") { ?>
...
<?php
$zx_bayar_kredit_view->Page_Terminate();
?>
