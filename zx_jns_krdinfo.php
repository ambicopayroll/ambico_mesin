...
//
// Table class for zx_jns_krd
//
class czx_jns_krd extends cTable {
	var $krd_id;
	var $krd_kode;
	var $krd_name;
	var $com_id;
	...
	function __construct() {
		...
		$this->TableVar = 'zx_jns_krd';
		$this->TableName = 'zx_jns_krd';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`zx_jns_krd`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// krd_id
		$this->krd_id = new cField('zx_jns_krd', 'zx_jns_krd', 'x_krd_id', 'krd_id', '`krd_id`', '`krd_id`', 16, -1, FALSE, '`krd_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// krd_kode
		$this->krd_kode = new cField('zx_jns_krd', 'zx_jns_krd', 'x_krd_kode', 'krd_kode', '`krd_kode`', '`krd_kode`', 200, -1, FALSE, '`krd_kode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->krd_kode->Sortable = TRUE; // Allow sort
		$this->fields['krd_kode'] = &$this->krd_kode;

		// krd_name
		$this->krd_name = new cField('zx_jns_krd', 'zx_jns_krd', 'x_krd_name', 'krd_name', '`krd_name`', '`krd_name`', 200, -1, FALSE, '`krd_name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->krd_name->Sortable = TRUE; // Allow sort
		$this->fields['krd_name'] = &$this->krd_name;

		// com_id
		$this->com_id = new cField('zx_jns_krd', 'zx_jns_krd', 'x_com_id', 'com_id', '`com_id`', '`com_id`', 2, -1, FALSE, '`com_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// lastupdate_date
		$this->lastupdate_date = new cField('zx_jns_krd', 'zx_jns_krd', 'x_lastupdate_date', 'lastupdate_date', '`lastupdate_date`', ew_CastDateFieldForLike('`lastupdate_date`', 0, "DB"), 135, 0, FALSE, '`lastupdate_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// lastupdate_user
		$this->lastupdate_user = new cField('zx_jns_krd', 'zx_jns_krd', 'x_lastupdate_user', 'lastupdate_user', '`lastupdate_user`', '`lastupdate_user`', 200, -1, FALSE, '`lastupdate_user`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`zx_jns_krd`";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('krd_id', $rs))
				ew_AddFilter($where, ew_QuotedName('krd_id', $this->DBID) . '=' . ew_QuotedValue($rs['krd_id'], $this->krd_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`krd_id` = @krd_id@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->krd_id->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@krd_id@", ew_AdjustSql($this->krd_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("zx_jns_krdview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("zx_jns_krdview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "zx_jns_krdadd.php?" . $this->UrlParm($parm);
		else
			$url = "zx_jns_krdadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("zx_jns_krdedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("zx_jns_krdadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("zx_jns_krddelete.php", $this->UrlParm());
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "krd_id:" . ew_VarToJson($this->krd_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->krd_id->CurrentValue)) {
			$sUrl .= "krd_id=" . urlencode($this->krd_id->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["krd_id"]))
				$arKeys[] = ew_StripSlashes($_POST["krd_id"]);
			elseif (isset($_GET["krd_id"]))
				$arKeys[] = ew_StripSlashes($_GET["krd_id"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->krd_kode->EditValue = $this->krd_kode->CurrentValue;
		...
		$this->krd_name->EditValue = $this->krd_name->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->krd_kode->Exportable) $Doc->ExportCaption($this->krd_kode);
					if ($this->krd_name->Exportable) $Doc->ExportCaption($this->krd_name);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->krd_kode->Exportable) $Doc->ExportField($this->krd_kode);
						if ($this->krd_name->Exportable) $Doc->ExportField($this->krd_name);
