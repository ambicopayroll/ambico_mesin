...
$zx_jns_krd_view = NULL; // Initialize page object first

class czx_jns_krd_view extends czx_jns_krd {
	...
	// Page object name
	var $PageObjName = 'zx_jns_krd_view';
	...
	function __construct() {
		...
		$KeyUrl = "";
		if (@$_GET["krd_id"] <> "") {
			$this->RecKey["krd_id"] = $_GET["krd_id"];
			$KeyUrl .= "&amp;krd_id=" . urlencode($this->RecKey["krd_id"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["krd_id"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["krd_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["krd_id"] <> "") {
				...
				$this->RecKey["krd_id"] = $this->krd_id->QueryStringValue;
			} elseif (@$_POST["krd_id"] <> "") {
				$this->krd_id->setFormValue($_POST["krd_id"]);
				$this->RecKey["krd_id"] = $this->krd_id->FormValue;
			} else {
				$sReturnUrl = "zx_jns_krdlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "zx_jns_krdlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "zx_jns_krdlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($zx_jns_krd_view)) $zx_jns_krd_view = new czx_jns_krd_view();

// Page init
$zx_jns_krd_view->Page_Init();

// Page main
$zx_jns_krd_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$zx_jns_krd_view->Page_Render();
?>
...
var CurrentForm = fzx_jns_krdview = new ew_Form("fzx_jns_krdview", "view");

// Form_CustomValidate event
fzx_jns_krdview.Form_CustomValidate = 
...
fzx_jns_krdview.ValidateRequired = true;
<?php } else { ?>
fzx_jns_krdview.ValidateRequired = false; 
<?php } ?>
...
<?php if ($zx_jns_krd->Export == "") { ?>
<div class="ewToolbar">
<?php if (!$zx_jns_krd_view->IsModal) { ?>
<?php if ($zx_jns_krd->Export == "") { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php } ?>
<?php $zx_jns_krd_view->ExportOptions->Render("body") ?>
<?php
	foreach ($zx_jns_krd_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$zx_jns_krd_view->IsModal) { ?>
<?php if ($zx_jns_krd->Export == "") { ?>
...
<?php } ?>
<?php $zx_jns_krd_view->ShowPageHeader(); ?>
<?php
$zx_jns_krd_view->ShowMessage();
?>
<form name="fzx_jns_krdview" id="fzx_jns_krdview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($zx_jns_krd_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $zx_jns_krd_view->Token ?>">
<?php } ?>
...
<?php if ($zx_jns_krd->krd_id->Visible) { // krd_id ?>
	<tr id="r_krd_id">
		<td><span id="elh_zx_jns_krd_krd_id"><?php echo $zx_jns_krd->krd_id->FldCaption() ?></span></td>
...
<?php echo $zx_jns_krd->krd_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_jns_krd->krd_kode->Visible) { // krd_kode ?>
	<tr id="r_krd_kode">
		<td><span id="elh_zx_jns_krd_krd_kode"><?php echo $zx_jns_krd->krd_kode->FldCaption() ?></span></td>
...
<?php echo $zx_jns_krd->krd_kode->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_jns_krd->krd_name->Visible) { // krd_name ?>
	<tr id="r_krd_name">
		<td><span id="elh_zx_jns_krd_krd_name"><?php echo $zx_jns_krd->krd_name->FldCaption() ?></span></td>
...
<?php echo $zx_jns_krd->krd_name->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_jns_krd->com_id->Visible) { // com_id ?>
	<tr id="r_com_id">
		<td><span id="elh_zx_jns_krd_com_id"><?php echo $zx_jns_krd->com_id->FldCaption() ?></span></td>
...
<?php echo $zx_jns_krd->com_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_jns_krd->lastupdate_date->Visible) { // lastupdate_date ?>
	<tr id="r_lastupdate_date">
		<td><span id="elh_zx_jns_krd_lastupdate_date"><?php echo $zx_jns_krd->lastupdate_date->FldCaption() ?></span></td>
...
<?php echo $zx_jns_krd->lastupdate_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($zx_jns_krd->lastupdate_user->Visible) { // lastupdate_user ?>
	<tr id="r_lastupdate_user">
		<td><span id="elh_zx_jns_krd_lastupdate_user"><?php echo $zx_jns_krd->lastupdate_user->FldCaption() ?></span></td>
...
<?php echo $zx_jns_krd->lastupdate_user->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($zx_jns_krd->Export == "") { ?>
<script type="text/javascript">
fzx_jns_krdview.Init();
</script>
<?php } ?>
<?php
$zx_jns_krd_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($zx_jns_krd->Export == "") { ?>
...
<?php
$zx_jns_krd_view->Page_Terminate();
?>
