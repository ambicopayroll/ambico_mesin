...
$zx_kredit_d_edit = NULL; // Initialize page object first

class czx_kredit_d_edit extends czx_kredit_d {
	...
	// Page object name
	var $PageObjName = 'zx_kredit_d_edit';
	...
	function Page_Main() {
		...
		if ($this->id_kredit->CurrentValue == "") {
			$this->Page_Terminate("zx_kredit_dlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// saldo_akh
			$this->saldo_akh->SetDbValueDef($rsnew, $this->saldo_akh->CurrentValue, 0, $this->saldo_akh->ReadOnly);

			// proses_bayar
			$this->proses_bayar->SetDbValueDef($rsnew, $this->proses_bayar->CurrentValue, 0, $this->proses_bayar->ReadOnly);
...
// Create page object
if (!isset($zx_kredit_d_edit)) $zx_kredit_d_edit = new czx_kredit_d_edit();

// Page init
$zx_kredit_d_edit->Page_Init();

// Page main
$zx_kredit_d_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$zx_kredit_d_edit->Page_Render();
?>
...
var CurrentForm = fzx_kredit_dedit = new ew_Form("fzx_kredit_dedit", "edit");

// Validate form
fzx_kredit_dedit.Validate = function() {
...
fzx_kredit_dedit.Form_CustomValidate = 
...
fzx_kredit_dedit.ValidateRequired = true;
<?php } else { ?>
fzx_kredit_dedit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$zx_kredit_d_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $zx_kredit_d_edit->ShowPageHeader(); ?>
<?php
$zx_kredit_d_edit->ShowMessage();
?>
<form name="fzx_kredit_dedit" id="fzx_kredit_dedit" class="<?php echo $zx_kredit_d_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($zx_kredit_d_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $zx_kredit_d_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $zx_kredit_d->id_kredit->EditValue ?></p></span>
</span>
<input type="hidden" data-table="zx_kredit_d" data-field="x_id_kredit" name="x_id_kredit" id="x_id_kredit" value="<?php echo ew_HtmlEncode($zx_kredit_d->id_kredit->CurrentValue) ?>">
...
<p class="form-control-static"><?php echo $zx_kredit_d->no_urut->EditValue ?></p></span>
</span>
<input type="hidden" data-table="zx_kredit_d" data-field="x_no_urut" name="x_no_urut" id="x_no_urut" value="<?php echo ew_HtmlEncode($zx_kredit_d->no_urut->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $zx_kredit_d_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fzx_kredit_dedit.Init();
</script>
<?php
$zx_kredit_d_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$zx_kredit_d_edit->Page_Terminate();
?>
