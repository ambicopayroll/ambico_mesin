...
$zx_kredit_m_edit = NULL; // Initialize page object first

class czx_kredit_m_edit extends czx_kredit_m {
	...
	// Page object name
	var $PageObjName = 'zx_kredit_m_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->id_kredit->CurrentValue == "") {
			$this->Page_Terminate("zx_kredit_mlist.php"); // Invalid key, return to list
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$this->no_kredit->SetDbValueDef($rsnew, $this->no_kredit->CurrentValue, "", $this->no_kredit->ReadOnly);

			// tgl_kredit
			$this->tgl_kredit->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->tgl_kredit->CurrentValue, 0), ew_CurrentDate(), $this->tgl_kredit->ReadOnly);

			// emp_id_auto
			$this->emp_id_auto->SetDbValueDef($rsnew, $this->emp_id_auto->CurrentValue, 0, $this->emp_id_auto->ReadOnly);

			// krd_id
			$this->krd_id->SetDbValueDef($rsnew, $this->krd_id->CurrentValue, 0, $this->krd_id->ReadOnly);

			// cara_hitung
			$this->cara_hitung->SetDbValueDef($rsnew, $this->cara_hitung->CurrentValue, 0, $this->cara_hitung->ReadOnly);

			// tot_kredit
			$this->tot_kredit->SetDbValueDef($rsnew, $this->tot_kredit->CurrentValue, 0, $this->tot_kredit->ReadOnly);

			// saldo_aw
			$this->saldo_aw->SetDbValueDef($rsnew, $this->saldo_aw->CurrentValue, 0, $this->saldo_aw->ReadOnly);

			// suku_bunga
			$this->suku_bunga->SetDbValueDef($rsnew, $this->suku_bunga->CurrentValue, 0, $this->suku_bunga->ReadOnly);

			// periode_bulan
			$this->periode_bulan->SetDbValueDef($rsnew, $this->periode_bulan->CurrentValue, 0, $this->periode_bulan->ReadOnly);

			// angs_pokok
			$this->angs_pokok->SetDbValueDef($rsnew, $this->angs_pokok->CurrentValue, 0, $this->angs_pokok->ReadOnly);

			// angs_pertama
			$this->angs_pertama->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->angs_pertama->CurrentValue, 0), ew_CurrentDate(), $this->angs_pertama->ReadOnly);

			// tot_debet
			$this->tot_debet->SetDbValueDef($rsnew, $this->tot_debet->CurrentValue, 0, $this->tot_debet->ReadOnly);

			// tot_angs_pokok
			$this->tot_angs_pokok->SetDbValueDef($rsnew, $this->tot_angs_pokok->CurrentValue, 0, $this->tot_angs_pokok->ReadOnly);

			// tot_bunga
			$this->tot_bunga->SetDbValueDef($rsnew, $this->tot_bunga->CurrentValue, 0, $this->tot_bunga->ReadOnly);

			// def_pembulatan
			$this->def_pembulatan->SetDbValueDef($rsnew, $this->def_pembulatan->CurrentValue, 0, $this->def_pembulatan->ReadOnly);

			// jumlah_piutang
			$this->jumlah_piutang->SetDbValueDef($rsnew, $this->jumlah_piutang->CurrentValue, 0, $this->jumlah_piutang->ReadOnly);

			// approv_by
			$this->approv_by->SetDbValueDef($rsnew, $this->approv_by->CurrentValue, "", $this->approv_by->ReadOnly);
			...
			// status_lunas
			$this->status_lunas->SetDbValueDef($rsnew, $this->status_lunas->CurrentValue, 0, $this->status_lunas->ReadOnly);
...
// Create page object
if (!isset($zx_kredit_m_edit)) $zx_kredit_m_edit = new czx_kredit_m_edit();

// Page init
$zx_kredit_m_edit->Page_Init();

// Page main
$zx_kredit_m_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$zx_kredit_m_edit->Page_Render();
?>
...
var CurrentForm = fzx_kredit_medit = new ew_Form("fzx_kredit_medit", "edit");

// Validate form
fzx_kredit_medit.Validate = function() {
...
fzx_kredit_medit.Form_CustomValidate = 
...
fzx_kredit_medit.ValidateRequired = true;
<?php } else { ?>
fzx_kredit_medit.ValidateRequired = false; 
<?php } ?>
...
<?php if (!$zx_kredit_m_edit->IsModal) { ?>
<div class="ewToolbar">
...
<?php } ?>
<?php $zx_kredit_m_edit->ShowPageHeader(); ?>
<?php
$zx_kredit_m_edit->ShowMessage();
?>
<form name="fzx_kredit_medit" id="fzx_kredit_medit" class="<?php echo $zx_kredit_m_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($zx_kredit_m_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $zx_kredit_m_edit->Token ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $zx_kredit_m->id_kredit->EditValue ?></p></span>
</span>
<input type="hidden" data-table="zx_kredit_m" data-field="x_id_kredit" name="x_id_kredit" id="x_id_kredit" value="<?php echo ew_HtmlEncode($zx_kredit_m->id_kredit->CurrentValue) ?>">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $zx_kredit_m_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
<script type="text/javascript">
fzx_kredit_medit.Init();
</script>
<?php
$zx_kredit_m_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$zx_kredit_m_edit->Page_Terminate();
?>
